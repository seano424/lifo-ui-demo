...
const createStoreInfoSchema = (
  ...
  z.object({
    store_name: createStoreNameValidator()
      .refine(
        (val) => val.length >= 1,
        t("storeInformation.validation.storeNameRequired")
      )
      .refine(
        (val) => val.length <= 100,
        t("storeInformation.validation.storeNameTooLong")
      ),
    business_name: z
      .string()
    ...
    store_type: z
      ...
      .nullable(),
    size_category: z
      .enum(["small", "medium", "large", "hypermarket"])
      .optional()
      .nullable(),
    address: z
      .string()
    ...
    city: z
      .string()
      .max(100, t("storeInformation.validation.cityTooLong"))
      .optional()
      .nullable(),
    postal_code: country
      ? createPostalCodeValidator(country)
          .optional()
          .nullable()
          .or(z.literal(""))
      : z
    ...
    phone: country
      ...
      : z
          .string()
          .max(20, t("storeInformation.validation.phoneTooLong"))
          .optional()
          .nullable(),
    email: createEmailValidator().optional().nullable().or(z.literal("")),
    website_url: createWebsiteValidator()
      .optional()
      .nullable()
      .or(z.literal("")),
    description: z
    ...
    waste_reduction_target_percent: z
      .number()
      .min(0)
      .max(100)
      .optional()
...
export default function StoreInformation({
  ...
  const {
    ...
    isLoading,
    error,
  } = useStoreSettings(effectiveStoreId || undefined);
  ...
  const form = useForm<StoreInfoFormData>({
    ...
    defaultValues: {
      ...
      waste_reduction_target_percent:
        DEFAULT_STORE_VALUES.WASTE_REDUCTION_TARGET,
    },
  });
    ...
    (clearUnsavedChanges: boolean = true) => {
      ...
      form.reset({
        ...
        default_markup_percent:
          ...
          DEFAULT_STORE_VALUES.MARKUP_PERCENT,
        waste_reduction_target_percent:
    ...
    try {
      ...
      reportError(
        error instanceof Error
          ? error
          : new Error("Failed to save store information"),
        {
          context: {
          ...
          <CardContent className="p-6">
            {userData?.requires_pin && (
              <Alert variant="destructive">
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  Store data not found. Please contact your administrator to
                  create a store.
                </AlertDescription>
              </Alert>
            )}
        ...
        <CardHeader className="py-4">
          <div className="flex items-center justify-between gap-2">
            <div className="flex flex-col gap-2">
              <Typography
                variant="h3"
                className="flex font-black items-center gap-2"
              >
          ...
          <form onSubmit={form.handleSubmit(handleSave)} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                <div className="flex flex-col gap-2">
                  <Label htmlFor="store_name">
                    {t("storeInformation.fields.storeName")}
                  </Label>
                  {isEditing ? (
                    <Input
                      id="store_name"
                ...
                <div className="flex flex-col gap-2">
                  <Label htmlFor="store_code">
                    {t("storeInformation.fields.storeCode")}
                  </Label>
                  {isEditing ? (
                    <Input
                      id="store_code"
                ...
                <div className="flex flex-col gap-2">
                  <Label htmlFor="store_type">
                    {t("storeInformation.fields.storeType")}
                  </Label>
                  {isEditing ? (
                    <Select
                      ...
                      onValueChange={(value) =>
                        form.setValue(
                          "store_type",
                          value as StoreInfoFormData["store_type"],
                          {
                            shouldDirty: true,
                          }
                        )
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue
                          placeholder={t(
                            "storeInformation.selectPlaceholders.storeType"
                          )}
                        />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <Typography variant="p">
                      {STORE_TYPES.find(
                        (type) => type.value === storeData?.store_type
                      )?.label || t("storeInformation.messages.notSet")}
                    </Typography>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <Label htmlFor="size_category">
                    {t("storeInformation.fields.sizeCategory")}
                  </Label>
                  {isEditing ? (
                    <Select
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue
                          placeholder={t(
                            "storeInformation.selectPlaceholders.sizeCategory"
                          )}
                        />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <Typography variant="p">
                      {SIZE_CATEGORIES.find(
                        (s) => s.value === storeData?.size_category
                      )?.label || t("storeInformation.messages.notSet")}
                    </Typography>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2 flex flex-col gap-2">
                  <Label htmlFor="address">
                    {t("storeInformation.fields.address")}
                  </Label>
                  {isEditing ? (
                    <Input
                      id="address"
                ...
                <div className="flex flex-col gap-2">
                  <Label htmlFor="city">
                    {t("storeInformation.fields.city")}
                  </Label>
                  {isEditing ? (
                    <Input
                      id="city"
                ...
                <div className="flex flex-col gap-2">
                  <Label htmlFor="postal_code">
                    {t("storeInformation.fields.postalCode")}
                  </Label>
                  {isEditing ? (
                    <Input
                      id="postal_code"
                      ...
                      placeholder={t(
                        "storeInformation.placeholders.postalCode"
                      )}
                    />
                  ) : (
                    <Typography variant="p">
                ...
                <div className="flex flex-col gap-2">
                  <Label htmlFor="country">
                    {t("storeInformation.fields.country")}
                  </Label>
                  {isEditing ? (
                    <Select
                      ...
                      onValueChange={(value) =>
                        form.setValue(
                          "country",
                          value as StoreInfoFormData["country"],
                          {
                            shouldDirty: true,
                          }
                        )
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue
                          placeholder={t(
                            "storeInformation.selectPlaceholders.country"
                          )}
                        />
                      </SelectTrigger>
                      <SelectContent>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col gap-2">
                  <Label htmlFor="phone">
                    {t("storeInformation.fields.phone")}
                  </Label>
                  {isEditing ? (
                    <Input
                      id="phone"
                ...
                <div className="flex flex-col gap-2">
                  <Label htmlFor="email">
                    {t("storeInformation.fields.email")}
                  </Label>
                  {isEditing ? (
                    <Input
                      id="email"
                ...
                <div className="md:col-span-2 flex flex-col gap-2">
                  <Label htmlFor="website_url">
                    {t("storeInformation.fields.website")}
                  </Label>
                  {isEditing ? (
                    <Input
                      id="website_url"
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-2">
                <Label htmlFor="description">
                  {t("storeInformation.fields.description")}
                </Label>
                {isEditing ? (
                  <Textarea
                    id="description"
                  ...
                  <Typography variant="p">
                    {storeData?.description ||
                      t("storeInformation.messages.noDescription")}
                  </Typography>
                )}
            ...
            {!permissions.canEditBasicInfo && !permissions.isLoading && (
              <Alert>
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  You have read-only access to store information. Contact your
                  store {permissions.isEmployee ? "manager or owner" : "owner"}{" "}
                  to make changes.
                </AlertDescription>
              </Alert>
            )}
