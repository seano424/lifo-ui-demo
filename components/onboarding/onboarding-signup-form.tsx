...
export function OnboardingSignUpForm({
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message ||
            errorData.error ||
            "Failed to create store and user records"
        );
      }
      ...
      if (error && typeof error === "object") {
        ...
        const allErrorText =
          `${errorMessage} ${errorDetails} ${errorHint}`.toLowerCase();
      ...
      setError(
        error instanceof Error ? error.message : t("errors.signupError")
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const storeName =
    selectedStoreForm?.store_name ||
    confirmedStoreInsert?.store_name ||
    "your store";
          ...
          <form onSubmit={handleSignUp}>
            <div className="flex flex-col gap-6">
              ...
              {isLoggedIn && (
                <Alert>
                  <AlertDescription>
                    You are logged in as {currentUser?.email}. This store will
