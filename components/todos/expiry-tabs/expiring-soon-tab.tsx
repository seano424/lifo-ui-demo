...
import { useExpiringSoonTodos } from '@/hooks/use-expiry-todos-tabs'
...
export function ExpiringSoonTab({ filters, pageSize = 20, onCountUpdate }: ExpiringSoonTabProps) {
  ...
  if (!isHydrated || isLoading) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-muted-foreground mt-2">{t('expiringSoon.loading')}</p>
      </div>
    )
  }
  ...
  if (isError) {
    return (
      <div className="text-center py-8">
        <p className="text-destructive">{t('expiringSoon.errorLoading')}</p>
  ...
  return (
    <TodoCardList
      ...
      emptyStateMessage={t('expiringSoon.noMatch')}
      emptyStateIcon=""
    />
  )
}
