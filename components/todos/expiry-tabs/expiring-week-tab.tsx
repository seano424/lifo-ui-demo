...
import { useExpiringWeekTodos } from '@/hooks/use-expiry-todos-tabs'
import { useTranslations } from 'next-intl'
import { useEffect, useRef, useState } from 'react'
import type { TodoFiltersState } from '../filters/types'
import { TodoCardList } from '../todo-card-list'

interface ExpiringWeekTabProps {
  filters: TodoFiltersState
  pageSize?: number
  onCountUpdate?: (count: number) => void
}

export function ExpiringWeekTab({ filters, pageSize = 20, onCountUpdate }: ExpiringWeekTabProps) {
  ...
  if (!isHydrated || isLoading) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-muted-foreground mt-2">{t('expiringWeek.loading')}</p>
      </div>
    )
  }
  ...
  if (isError) {
    return (
      <div className="text-center py-8">
        <p className="text-destructive">{t('expiringWeek.errorLoading')}</p>
