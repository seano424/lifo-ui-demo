    ...
    switch (status) {
      case 'completed':
        return {
          ...
          className: 'bg-primary-100 text-primary-800 dark:bg-primary-900/30 dark:text-primary-400',
        }
      case 'in_progress':
        return {
          icon: ClockIcon,
        ...
        <div className="flex w-full overflow-x-auto px-4 pb-8 scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent  gap-1 justify-between pt-2">
          {tabs.map(tab => (
            <button
              ...
              className={cn(
                'transition-all flex flex-col items-center gap-1  font-heading flex-1 text-xs sm:text-base hover:bg-opacity-0 shrink-0 py-2 rounded-2xl',
                activeTab === tab.id &&
