...
import { Loader2, AlertTriangle, ArrowRightFromLine } from 'lucide-react'
...
export function StoreInsightsDashboard({ storeId: propStoreId }: StoreInsightsDashboardProps) {
  ...
  const { expiring_soon, ready_for_discount, perfect_for_donation, high_urgency } = data

  return (
    <div className="space-y-6 border shadow-xs rounded px-8 py-4">
      ...
      <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-8 text-center sm:text-left">
        <div className="flex flex-col gap-2">
          <Typography variant="h4">Expiring Soon</Typography>
          <Typography variant="p">Batches expiring in 1-2 days</Typography>
        ...
        <div className="flex flex-col gap-2">
          ...
          <Typography variant="p">Needs immediate attention</Typography>
          <Typography variant="p">{high_urgency.count}</Typography>
        </div>
      </div>

      {/* TODO: Add TODO Page and this can be added there... */}
      {/* Summary Card */}
      {/* {total_actionable_items > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {total_actionable_items} actionable items of {total_active_batches} active batches
            </CardDescription>
          </CardHeader>
          <CardContent>
