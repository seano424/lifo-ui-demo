...
export function FilterDemo() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filter Panel */}
      <TodoFiltersPanel filters={filters} onFiltersChange={setFilters} isLoading={isLoading} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 border rounded-lg">
          ...
          <p className="text-sm text-muted-foreground">With current filters</p>
        </div>
        ...
        <div className="p-4 border rounded-lg">
          ...
          <p className="text-sm text-muted-foreground">Currently being discounted</p>
        </div>

        <div className="p-4 border rounded-lg">
          <h3 className="font-semibold">Recently Completed</h3>
          <p className="text-2xl font-bold text-green-600">{recentCompleted?.length || 0}</p>
          <p className="text-sm text-muted-foreground">All completed todos</p>
        </div>
      </div>

      {/* Debug Info */}
      <details className="border rounded p-4">
        <summary className="cursor-pointer font-semibold">Debug: Filter State</summary>
