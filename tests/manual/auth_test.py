    ...
    for user_id, store_id in zip(test_users, test_stores):
        ...
        try:
            ...
            for endpoint in auth_endpoints:
                status_code, response_data = test_authenticated_endpoint(endpoint, token)
                
                if status_code:
                    status_icon = "✅" if status_code in [200, 404] else "⚠️" if status_code == 403 else "❌"
                    endpoint_name = endpoint.split('/')[-2] + "/" + endpoint.split('/')[-1][:8]
                    print(f"  {status_icon} {endpoint_name}: {status_code}")
                    
                    # Show response details for successful requests
                    if status_code == 200 and isinstance(response_data, dict):
                        if 'data' in response_data:
                            print(f"      📊 Data returned: {len(response_data['data'])} items")
                        elif 'message' in response_data:
                            print(f"      📝 Message: {response_data['message'][:60]}...")
                    elif status_code == 403:
                        print(f"      🔒 Still forbidden - may need different auth or permissions")
