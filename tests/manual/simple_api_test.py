...
def test_endpoint(endpoint, method="GET", expected_codes=[200]):
    ...
    try:
        ...
        print(f"{status} {method} {endpoint} - {response.status_code} ({response_time:.0f}ms)")
...
def main():
    ...
    for endpoint in endpoints:
        ...
        success, response_time, status_code = test_endpoint(endpoint, expected_codes=expected)
    ...
    avg_response_time = sum(r['response_time'] for r in results if r['success']) / max(1, passed_tests)
