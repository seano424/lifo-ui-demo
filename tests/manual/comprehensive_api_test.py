...
class ComprehensiveAPITester:
    ...
    def log_test(self, test_name: str, passed: bool, response_time: float, 
                 status_code: Optional[int] = None, error: Optional[str] = None,
                 details: Optional[Dict] = None, response_data: Optional[Dict] = None):
        ...
        if error and not passed:
            ...
            if isinstance(response_data, dict):
                if 'status' in response_data:
                    ...
                    print(f"      ℹ️ Response keys: {', '.join(list(response_data.keys())[:5])}")
    ...
    def test_health_endpoints(self):
        ...
        for endpoint, name in health_endpoints:
            try:
                response, response_time, response_data = self.make_request('GET', endpoint)
                self.log_test(
                    f"Health - {name}",
    ...
    def test_scoring_endpoints(self):
        ...
        for store_id in TEST_STORES:
            ...
            scoring_endpoints = [
                (f"{API_V1_PREFIX}/scoring/batch/{store_id}", f"Batch Scoring - {store_name}"),
                (f"{API_V1_PREFIX}/scoring/alerts/{store_id}", f"Scoring Alerts - {store_name}"),
                (f"{API_V1_PREFIX}/scoring/recommendations/{store_id}", f"Recommendations - {store_name}"),
                (f"{API_V1_PREFIX}/scoring/analytics/{store_id}", f"Scoring Analytics - {store_name}")
            ]
    ...
    def test_analytics_endpoints(self):
        ...
        for store_id in TEST_STORES:
            ...
            analytics_endpoints = [
                (f"{API_V1_PREFIX}/analytics/store/{store_id}", f"Store Analytics - {store_name}"),
                (f"{API_V1_PREFIX}/analytics/dashboard/{store_id}", f"Dashboard Data - {store_name}"),
                (f"{API_V1_PREFIX}/analytics/performance/{store_id}", f"Performance Metrics - {store_name}"),
                (f"{API_V1_PREFIX}/analytics/trends/{store_id}", f"Trends Analysis - {store_name}"),
                (f"{API_V1_PREFIX}/analytics/exports/{store_id}", f"Data Exports - {store_name}")
            ]
    ...
    def test_csv_endpoints(self):
        ...
        for store_id in TEST_STORES:
            ...
            csv_endpoints = [
                (f"{API_V1_PREFIX}/csv/validate/{store_id}", f"CSV Validation - {store_name}"),
                (f"{API_V1_PREFIX}/csv/template/{store_id}", f"CSV Template - {store_name}"),
                (f"{API_V1_PREFIX}/csv/status/{store_id}", f"CSV Status - {store_name}")
            ]
    ...
    def test_mobile_endpoints(self):
        ...
        for store_id in TEST_STORES:
            ...
            mobile_endpoints = [
                (f"{API_V1_PREFIX}/mobile/session/{store_id}", f"Mobile Session - {store_name}"),
                (f"{API_V1_PREFIX}/mobile/quick-score/{store_id}", f"Quick Score - {store_name}"),
                (f"{API_V1_PREFIX}/mobile/sync/{store_id}", f"Mobile Sync - {store_name}")
            ]
            ...
            for endpoint, name in mobile_endpoints:
                try:
                    ...
                    self.log_test(
                        ...
                        response_time,
                        response.status_code,
                        details={'mobile_performance_target_met': mobile_performance_ok},
                        response_data=response_data
                    )
                    
                    if not mobile_performance_ok and success:
                        print(f"      ⚠️ Performance: {response_time*1000:.0f}ms > 300ms target")
    ...
    def test_vision_ocr_endpoints(self):
        ...
        for endpoint, name in vision_endpoints:
            try:
                response, response_time, response_data = self.make_request('GET', endpoint)
                success = response.status_code in [200, 401, 404, 405, 422]  # 405 = Method not allowed for GET
                self.log_test(
                    name,
                    success,
                    response_time,
    ...
    def test_donation_endpoints(self):
        ...
        for store_id in TEST_STORES:
            ...
            donation_endpoints = [
                (f"{API_V1_PREFIX}/donations/eligible/{store_id}", f"Eligible Donations - {store_name}"),
                (f"{API_V1_PREFIX}/donations/recipients/{store_id}", f"Recipients - {store_name}"),
                (f"{API_V1_PREFIX}/donation-queries/stats/{store_id}", f"Donation Stats - {store_name}")
            ]
    ...
    def test_batch_endpoints(self):
        ...
        for store_id in TEST_STORES:
            ...
            batch_endpoints = [
                (f"{API_V1_PREFIX}/batches/scan/{store_id}", f"Scan Batches - {store_name}"),
                (f"{API_V1_PREFIX}/batches/recent/{store_id}", f"Recent Batches - {store_name}"),
                (f"{API_V1_PREFIX}/scan/workflow/{store_id}", f"Scan Workflow - {store_name}")
            ]
    ...
    def test_mvp_endpoints(self):
        ...
        for store_id in TEST_STORES:
            ...
            mvp_endpoints = [
                (f"{API_V1_PREFIX}/mvp/dashboard/{store_id}", f"MVP Dashboard - {store_name}"),
                (f"{API_V1_PREFIX}/mvp/quick-stats/{store_id}", f"Quick Stats - {store_name}")
            ]
    ...
    def test_error_handling(self):
        ...
        error_test_cases = [
            ...
            (f"{API_V1_PREFIX}/analytics/store/invalid-store-id", "Invalid Store ID Format"),
            # Non-existent store ID
            (f"{API_V1_PREFIX}/analytics/store/99999999-9999-9999-9999-999999999999", "Non-existent Store ID"),
            # Invalid endpoint
    ...
    def run_performance_tests(self):
        ...
        if successful_requests:
            ...
            self.log_test("Performance - Concurrent Requests", False, 0, error="No successful requests")
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        success_rate = round(self.test_results['passed_tests'] / max(1, self.test_results['total_tests']) * 100, 2)
        ...
        for error in self.test_results['error_details']:
            error_type = f"HTTP {error['status_code']}" if error['status_code'] else "Network Error"
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if success_rate < 0.5:
            recommendations.append("🚨 CRITICAL: Less than 50% of endpoints are working - immediate attention required")
        elif success_rate < 0.8:
            recommendations.append("⚠️ Multiple endpoint failures detected - review error logs")
        ...
        mobile_tests = [t for t in self.test_results['test_details'] if 'Mobile' in t['test_name']]
        slow_mobile = [t for t in mobile_tests if t['response_time_ms'] > 300]
        if slow_mobile:
            recommendations.append(f"📱 Optimize {len(slow_mobile)} mobile endpoint(s) for <300ms target")
        
        # Error pattern analysis
        error_404_count = len([e for e in self.test_results['error_details'] if e['status_code'] == 404])
        if error_404_count > 10:
            recommendations.append("🔍 Many 404 errors - check endpoint routing configuration")
        
        # Coverage analysis
        total_endpoints = len(self.test_results['endpoint_coverage'])
        if total_endpoints < 20:
            recommendations.append("📊 Consider testing more endpoint variations for better coverage")
    ...
    def run_all_tests(self):
        ...
        if report['performance_metrics']:
            ...
            for endpoint, stats in report['performance_metrics'].items():
                ...
                print(f"   {endpoint}: {stats['avg_response_time_ms']}ms avg {mobile_status}")
        ...
        report_file = f"/home/slim/lifo-app/comprehensive_api_test_report_{timestamp}.json"
