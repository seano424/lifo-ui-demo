...
class APITester:
    def __init__(self, base_url: str = BASE_URL):
        ...
        self.session.headers.update({
            ...
            'User-Agent': 'LIFO-API-Tester/1.0'
        })
    ...
    def test_health_endpoints(self):
        ...
        for endpoint in health_endpoints:
            try:
                ...
                self.log_test(
                    test_name,
                    response.status_code in [200, 503],  # 503 is acceptable for health checks
                    response_time,
                    response.status_code
                )
            except Exception as e:
                self.log_test(f"Health Check: {endpoint.split('/')[-1]}", False, 0, error=str(e))
    ...
    def test_mobile_endpoints(self):
        ...
        for endpoint in mobile_endpoints:
            try:
                ...
                status_ok = response.status_code in [200, 401, 422]  # 401 if auth required, 422 if params needed
                ...
                self.log_test(f"Mobile: {endpoint.split('/')[-1]}", False, 0, error=str(e))
    ...
    def test_csv_endpoints(self):
        ...
        try:
            response, response_time = self.make_request('GET', f"{API_V1_PREFIX}/csv/info")
            self.log_test(
                "CSV Info Endpoint",
                response.status_code == 200,
                response_time,
        ...
        try:
            response, response_time = self.make_request('GET', f"{API_V1_PREFIX}/csv/sample")
            self.log_test(
                "CSV Sample Template",
                response.status_code == 200,
                response_time,
    ...
    def test_analytics_endpoints(self):
        ...
        for endpoint in analytics_endpoints:
            try:
                ...
                self.log_test(
                    test_name,
                    response.status_code in [200, 401],  # 401 if authentication required
                    response_time,
                    response.status_code
                )
            except Exception as e:
                self.log_test(f"Analytics: {endpoint.split('/')[-1]}", False, 0, error=str(e))
    ...
    def test_vision_ocr_endpoints(self):
        ...
        for endpoint in vision_endpoints:
            try:
                ...
                self.log_test(f"Vision: {endpoint.split('/')[-1]}", False, 0, error=str(e))
    ...
    def test_donation_endpoints(self):
        ...
        for endpoint in donation_endpoints:
            try:
                ...
                self.log_test(f"Donations: {endpoint.split('/')[-1]}", False, 0, error=str(e))
    ...
    def test_error_handling(self):
        ...
        try:
            response, response_time = self.make_request(
                'POST', f"{API_V1_PREFIX}/mobile/scan-session",
                data="invalid json",
    ...
    def run_performance_tests(self):
        ...
        if successful_requests:
            ...
            self.log_test("Concurrent Health Requests (10x)", False, 0, error="No successful requests")
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        # Check performance
        slow_tests = [t for t in self.test_results['test_details'] if t['response_time_ms'] > 500]
        if slow_tests:
            recommendations.append(f"Optimize {len(slow_tests)} slow endpoint(s) (>500ms)")
        ...
        if success_rate < 0.9:
            recommendations.append("Improve overall API reliability (success rate < 90%)")
