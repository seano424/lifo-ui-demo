...
export async function getStoreThreshold(
  ...
  try {
    ...
    const threshold =
      type === 'critical' ? settings?.critical_threshold || 0.8 : settings?.warning_threshold || 0.7
...
export async function getStoreThresholds(
  ...
  try {
    ...
    return {
      critical: Number.isNaN(critical) || critical < 0 || critical > 1 ? 0.8 : critical,
      warning: Number.isNaN(warning) || warning < 0 || warning > 1 ? 0.7 : warning,
    }
  } catch (error) {
