-- Migration: Fix p_exclude_drafts logic to allow draft/ignored batches when explicitly filtered
--
-- Problem 1: When p_status='draft' or p_status='ignored', the p_exclude_drafts filter
-- was preventing these batches from appearing even though the user explicitly requested them.
--
-- Problem 2: Date filters (p_expiring_in_days, p_expiry_date_from, p_expiry_date_to) were
-- excluding batches with NULL expiry dates, which prevented draft/ignored batches from
-- appearing even when explicitly filtered for.
--
-- Solution:
-- 1. Don't exclude draft/ignored batches when user explicitly filters for them
-- 2. Allow NULL expiry dates when user explicitly filters for draft/ignored status
...
-- Recreate with fixed p_exclude_drafts logic
...
BEGIN
  ...
  WITH filtered_batches AS (
    ...
    WHERE b.store_id = p_store_id
      ...
      -- FIXED: Don't exclude draft/ignored when user explicitly filters for them
      AND (
        p_exclude_drafts IS NULL
        OR NOT p_exclude_drafts
        OR p_status IN ('draft', 'ignored')  -- Override: if user explicitly filters for these, show them
      ...
      -- Date filters: allow NULL expiry dates when explicitly filtering for draft/ignored
      AND (
        p_expiring_in_days IS NULL
        OR p_status IN ('draft', 'ignored')  -- Allow NULL dates when explicitly viewing draft/ignored
...
COMMENT ON FUNCTION inventory.get_batches_paginated(
  ...
  character varying, character varying, text, boolean, integer,
  date, date, date, date, character varying, character varying
) IS 'Enhanced RPC for paginated batch retrieval with comprehensive search. Supports filtering out draft/ignored batches via p_exclude_drafts parameter, but automatically shows them when user explicitly filters for those statuses.';
