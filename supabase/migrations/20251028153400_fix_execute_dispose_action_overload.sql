-- Migration: Fix execute_dispose_action function overloading
--
-- Problem: PostgREST error PGRST203 - Multiple overloaded versions of execute_dispose_action exist
-- The production database has two versions:
--   1. execute_dispose_action(p_batch_id, p_quantity_disposed, p_disposal_reason, p_user_id, p_notes) [5 params]
--   2. execute_dispose_action(p_batch_id, p_quantity_disposed, p_disposal_reason, p_user_id, p_notes, p_recommended_action) [6 params]
--
...
-- This matches the migration already applied to production by Claude Desktop.
--
...
DROP FUNCTION IF EXISTS public.execute_dispose_action(
  p_batch_id uuid,
...
COMMENT ON FUNCTION public.execute_dispose_action(uuid, numeric, text, uuid, text, text) IS
  'Executes disposal action for a batch. Removed duplicate 5-parameter overload to fix function resolution ambiguity. The p_recommended_action parameter is optional and can be used to track AI recommendations.';
