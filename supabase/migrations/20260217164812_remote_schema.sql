...
CREATE OR REPLACE FUNCTION inventory.get_product_detail(p_product_id uuid, p_store_id uuid)
 RETURNS TABLE(product_id uuid, name character varying, brand character varying, category_id uuid, barcode text, typical_shelf_life_days integer, base_cost_price numeric, base_selling_price numeric, description text, image_url text, sku character varying, unit_type character varying, created_at timestamp without time zone, updated_at timestamp without time zone, created_by uuid, last_verified timestamp without time zone, open_food_facts_data jsonb, store_cost_price numeric, store_selling_price numeric, store_is_active boolean, store_sku character varying, supplier_code character varying, shelf_life_override_days integer, square_quantity numeric, square_quantity_updated_at timestamp without time zone, category_code text, category_display_name text, category_display_name_fr text, category_display_name_nl text, category_typical_shelf_life_days integer, category_default_shelf_life_days integer, total_stock numeric, active_batches_count bigint)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT
    ...
    NULL::text           AS category_display_name_nl,  -- column does not exist yet
  ...
  LEFT JOIN inventory.store_category_settings scs
    ON scs.store_id = sp.store_id AND scs.category_id = p.category_id
  LEFT JOIN LATERAL (
    SELECT
    ...
    WHERE b.store_id = sp.store_id AND b.product_id = sp.product_id
  ) ba ON true
