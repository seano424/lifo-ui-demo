-- Migration: Recreate get_products_paginated using store_inventory_stats view
--
-- Context: Previous migration removed total_stock, active_batches_count, avg_days_to_expiry
-- from products table and dropped get_products_paginated function. Frontend still needs it.
--
-- This migration recreates the function using the store_inventory_stats view instead
-- of the removed denormalized columns.
...
COMMENT ON FUNCTION inventory.get_products_paginated(
  uuid, text, text, text, text, text, integer, integer
) IS 'Paginated product listing with filtering and search. Uses store_inventory_stats view for real-time inventory metrics instead of denormalized columns.';

-- Recreate public wrapper (note: p_search parameter not exposed in public wrapper in original schema)
...
COMMENT ON FUNCTION public.get_products_paginated(
  uuid, text, text, text, text, integer, integer
) IS 'Public wrapper for inventory.get_products_paginated. Uses store_inventory_stats view for metrics.';
