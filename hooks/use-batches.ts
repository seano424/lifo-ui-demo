...
/**
 * Hook for fetching draft batches (batches without expiry dates)
 * These are batches created from CSV uploads where no expiry date was provided
 * and need to be completed with OCR or manual date entry
 */
export function useDraftBatches(pageSize: number = 50) {
  const activeStoreId = useActiveStoreId()

  const result = useInfiniteQuery({
    queryKey: queryKeys.batches.infinite(activeStoreId || '', { status: 'draft' }),
    queryFn: ({ pageParam = 0 }) =>
      fetchBatchesPageRPC(
        { page: pageParam, pageSize },
        {
          ...
          status: 'draft',
          // Don't exclude drafts - we specifically want them!
          excludeDrafts: false,
        },
        undefined,
      ),
    ...
    retry: (failureCount, error: SupabaseError) => {
      if (error?.message?.includes('failed to parse order')) {
        console.error('[useDraftBatches] PostgREST ordering error - not retrying:', error)
        return false
      }
      if (error?.status && error.status >= 400 && error.status < 500 && error.status !== 408) {
        return false
