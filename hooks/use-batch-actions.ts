...
import { getSectionsToInvalidateAfterAction } from '@/lib/queries/todos-rpc'
...
interface DonatePreparedActionParams {
  p_batch_id: string
  p_quantity_affected: number
  p_preparation_details?: string | null
  p_user_id: string
  p_notes?: string | null
}
...
interface DonatePreparedParams {
  batchId: string
  quantity: number
...
// Response types for optimistic updates
...
export function useBatchActions() {
  ...
  // ENHANCED: Combined invalidation strategy (original + sections)
  const invalidateRelatedQueries = async (
    ...
    if (storeId) {
      ...
      // Always invalidate dashboard summary for sections
      queryClient.invalidateQueries({
        queryKey: queryKeys.todos.dashboardSummary(storeId),
      })

      // Original comprehensive invalidation (preserved for backward compatibility)
      await Promise.all([
        ...
        // Original todos state queries (preserved for backward compatibility)
        queryClient.invalidateQueries({
  ...
  // 6. DONATE PREPARED ACTION (NEW - added to match the 5 action types)
  const executeDonatePrepared = useMutation({
    ...
    onSuccess: async (result, variables) => {
      if (result.success) {
        toast.success(`Prepared ${variables.quantity} units for donation`)
        await invalidateRelatedQueries(variables.batchId, 'donate_prepared')
      } else {
        toast.error(result.error || 'Preparation failed')
      }
    },
    onError: () => {
      toast.error('Failed to prepare for donation')
    },
  })
  ...
  const markAsDisposed = (batchId: string, reason: string, quantity?: number) =>
    executeDispose.mutate({
      batchId,
