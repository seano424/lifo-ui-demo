...
class CSVTestSuite:
    ...
    def test_server_connectivity(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.log_test(
                    "Server Connectivity", True, f"Server healthy: {data.get('status')}"
                )
                return True
            else:
                self.log_test(
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                ...
                response = self.make_request(
                    "POST", "/api/v1/csv-upload/upload", files=files, data=data
                )
    ...
    def test_csv_validation(self, file_key: str, expected_success: bool = True):
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                ...
                response = self.make_request(
                    "POST", "/api/v1/csv-upload/validate", files=files, data=data
                )
    ...
    def test_csv_batch_creation(self, file_key: str, chunk_size: int = 50):
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                files = {"file": (file_path.name, f, "text/csv")}
                data = {"store_id": self.store_id, "chunk_size": str(chunk_size)}

                response = self.make_request(
                    "POST",
                    "/api/v1/csv-upload/upload-and-create-batches",
                    files=files,
                    data=data,
                )
    ...
    def test_authentication_and_authorization(self):
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                ...
                response = session.post(
                    f"{self.base_url}/api/v1/csv-upload/upload",
                    files=files,
                    data=data,
                    timeout=10,
                )
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                ...
                response = self.make_request(
                    "POST", "/api/v1/csv-upload/upload", files=files
                )

            if response.status_code == 422:
                self.log_test(
                    "Authorization - Missing Store ID",
                    True,
    ...
    def test_invalid_inputs(self):
        ...
        try:
            ...
            if response.status_code == 400:
                self.log_test(
                    "Invalid Input - File Type", True, "Correctly rejected non-CSV file"
                )
            else:
                self.log_test(
    ...
    def print_summary(self):
        ...
        if failed == 0:
            ...
            print(
                "❌ MULTIPLE FAILURES - Check your API configuration and dependencies."
            )
...
def main():
    ...
    if missing_files:
        ...
        print(
            "   Make sure all CSV test files are in the same directory as this script"
        )
