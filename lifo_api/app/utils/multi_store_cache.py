...
class MultiStoreCache:
    ...
    def _generate_key(
        self, user_id: str, operation: str, params: dict[str, Any] | None = None
    ) -> str:
    ...
    async def set(
        ...
        try:
            cache_key = self._generate_key(user_id, operation, params)
            ttl = timedelta(
                minutes=ttl_minutes or self._default_ttl.total_seconds() / 60
            )
    ...
    async def invalidate_user(self, user_id: str) -> None:
        ...
        try:
            user_keys = [
                k for k in self._cache.keys() if f"multi_store:{user_id}:" in k
            ]
    ...
    async def invalidate_operation(self, operation: str) -> None:
        ...
        try:
            operation_keys = [
                ...
                if f":{operation}:" in k or k.endswith(f":{operation}")
            ]
            ...
            logger.warning(
                "Multi-store cache operation invalidation failed", error=str(e)
            )
...
def get_multi_store_cache() -> MultiStoreCache:
    ...
    if _multi_store_cache is None:
        _multi_store_cache = MultiStoreCache(
            default_ttl_minutes=5
        )  # 5-minute default TTL for MVP
