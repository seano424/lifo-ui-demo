...
class DatabaseHealthMonitor:
    ...
    async def log_operation_metrics(self, operation: str, success: bool, duration_ms: float, error: Optional[Exception] = None):
        """
...
async def create_fresh_session():
    """
    Create a fresh database session with health monitoring
    ...
    # Verify session health before returning
    is_healthy = await database_health_monitor.check_session_health(session)
    
    if not is_healthy:
        await session.close()
        logger.warning("Created session failed health check, attempting new session")
        # Try once more
        session = async_session_factory()
        is_healthy = await database_health_monitor.check_session_health(session)
        ...
        if not is_healthy:
            await session.close()
            raise Exception("Unable to create healthy database session")
