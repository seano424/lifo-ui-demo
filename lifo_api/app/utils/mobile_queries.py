...
class MobileQueryOptimizer:
    ...
    async def get_store_inventory_fast(
        ...
        if urgency_filter == "critical":
            ...
            base_query += " AND b.best_before_date <= (CURRENT_DATE + INTERVAL '3 days')"
        elif urgency_filter == "medium":
            base_query += " AND b.best_before_date <= (CURRENT_DATE + INTERVAL '7 days')"
    ...
    async def get_batch_quick_score_data(self, batch_id: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for row in rows:
                categories.append({
                    ...
                    "urgency_ratio": (row.urgent_count / row.batch_count) if row.batch_count > 0 else 0
                })
...
class MobileQueryMonitor:
    ...
    def record_query(self, query_name: str, execution_time_ms: float, result_count: int):
        ...
        stats["avg_results"] = ((stats["avg_results"] * (stats["total_executions"] - 1)) + result_count) / stats["total_executions"]
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        for query_name, stats in self.query_stats.items():
            if stats["total_executions"] > 0:
                report[query_name] = {
                    "avg_execution_ms": stats["total_time_ms"] / stats["total_executions"],
                    "total_executions": stats["total_executions"],
                    "slow_query_rate": stats["slow_queries"] / stats["total_executions"],
                    ...
                    "meets_mobile_target": (stats["total_time_ms"] / stats["total_executions"]) <= self.slow_query_threshold
                }
