...
class TimingMetrics:
    ...
    def calculate_throughput(self):
        ...
        if self.total_processing_ms > 0:
            self.items_per_second = (
                self.items_processed / self.total_processing_ms
            ) * 1000
        return self.items_per_second

    def add_slow_operation(
        self, operation: str, duration_ms: float, threshold_ms: float = 100
    ):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_chunk_time_ms": round(
                sum(self.chunk_timings) / len(self.chunk_timings), 2
            )
            if self.chunk_timings
            else 0,
    ...
    def measure(self, operation: str, threshold_ms: float = 100):
        ...
        try:
            ...
            if duration_ms > 50:  # Log operations over 50ms
                logger.debug(
                    f"Operation timing: {operation}", duration_ms=round(duration_ms, 2)
                )
...
class CSVProcessingTimer(PerformanceTimer):
    ...
    def end_stage(self, stage: str) -> float:
        ...
        logger.info(f"Completed stage: {stage}", duration_ms=round(duration_ms, 2))
    ...
    def get_stage_summary(self) -> dict[str, Any]:
        ...
        total_ms = self.metrics.total_processing_ms or sum(
            self.metrics.sub_timings.values()
        )
        ...
        for stage in self.stage_order:
            ...
            if stage_ms > 0:
                stages[stage] = {
                    ...
                    "percentage": round((stage_ms / total_ms) * 100, 1)
                    if total_ms > 0
                    else 0,
                }
        ...
        return {
            ...
            "slowest_stage": max(stages.items(), key=lambda x: x[1]["duration_ms"])[0]
            if stages
            else None,
        }
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            if isinstance(result, dict):
                ...
                result["performance_metrics"][f"{operation_name}_ms"] = (
                    round(duration_ms, 2) if duration_ms else 0
                )
...
class PerformanceMonitor:
    ...
    def get_all_statistics(self) -> dict[str, Any]:
        ...
        return {op: self.get_statistics(op) for op in self.operations}
