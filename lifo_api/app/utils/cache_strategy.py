    ...
    async def _set_l1(self, key: str, data: dict, cache_type: str):
        ...
        if len(self.l1_cache) >= config["max_size"]:
            ...
            oldest_key = min(self.l1_cache.keys(), 
                           key=lambda k: self.l1_cache[k].get("accessed", 0))
    ...
    def get_stats(self) -> dict:
        ...
        for cache_type, data in self.cache_stats.items():
            ...
            stats[cache_type] = {
                ...
                "l1_size": len([k for k, v in self.l1_cache.items() 
...
class CacheWarmer:
        ...
        try:
            ...
            for store in stores:
                ...
                warming_tasks.append(
                    self._warm_mobile_cache(store.id, read_ops)
                )
                
                # Warm analytics cache
                warming_tasks.append(
                    self._warm_analytics_cache(store.id, read_ops)
                )
    ...
    async def _warm_analytics_cache(self, store_id: str, read_ops):
        ...
        try:
            ...
            logger.error(
                "Failed to warm analytics cache",
                store_id=store_id,
                error=str(e)
            )
            return False
