...
class SeasonalAdjustmentService:
    ...
    def calculate_seasonal_adjustment(
        self, category: str, current_date: datetime | None = None
    ) -> float:
        """
    ...
    def apply_seasonal_adjustment(
        self, base_score: float, category: str, current_date: datetime | None = None
    ) -> float:
        """
...
class EnhancedVelocityAnalyzer:
    ...
    def calculate_sales_trend(
        self, historical_sales: list[float], lookback_days: int = 7
    ) -> float:
        """
        ...
        recent_sales = (
            historical_sales[-lookback_days:]
            if len(historical_sales) > lookback_days
            else historical_sales
        )
    ...
    def calculate_velocity_score_enhanced(
        ...
        if avg_daily_sales <= 0:
            ...
            sales_trend = (
                self.calculate_sales_trend(historical_sales)
                if historical_sales
                else 0.0
            )
...
class IntelligentScoringCache:
    ...
    def get_cached_score(self, batch_id: str, context: dict) -> dict[str, Any] | None:
        """
        ...
        self.access_history.append(
            {"batch_id": batch_id, "timestamp": datetime.now(), "hit": True}
        )
    ...
    def store_score(self, batch_id: str, context: dict, score_result: dict):
        """
        ...
        self.logger.debug("Score cached", batch_id=batch_id, cache_size=len(self.cache))
    ...
    def get_cache_statistics(self) -> dict[str, Any]:
        ...
        hot_items = sorted(self.cache_hits.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]

        cold_items = sorted(
            self.cache_misses.items(), key=lambda x: x[1], reverse=True
        )[:10]
...
class EnhancedBulkDetector:
    ...
    def detect_bulk_mismatch(
        ...
        if avg_daily_sales and avg_daily_sales > 0:
            ...
            estimated_daily_sales = historical_velocity.get(
                "p50", current_quantity * self.velocity_factors.get(category, 0.1)
            )
        else:
            # Fallback to category-based estimation
            estimated_daily_sales = current_quantity * self.velocity_factors.get(
                category, 0.1
            )

        # Calculate sellout days
        sellout_days = (
            current_quantity / estimated_daily_sales
            if estimated_daily_sales > 0
            else 999
        )
    ...
    def calculate_optimal_split(
        self, total_quantity: float, days_to_expiry: int, category: str
    ) -> dict[str, Any]:
        """
        ...
        if total_quantity < threshold:
            return {"should_split": False, "total_quantity": total_quantity}
...
class OptimizedScoringOrchestrator:
    ...
    def score_with_optimizations(
        ...
        cache_context = {
            ...
            "urgency_level": self._determine_urgency_level(
                days_to_expiry, base_scores.get("composite_score", 0)
            ),
        }
        ...
        if historical_data and historical_data.get("sales_history"):
            enhanced_velocity, velocity_metadata = (
                self.velocity_analyzer.calculate_velocity_score_enhanced(
        ...
        bulk_analysis = self.bulk_detector.detect_bulk_mismatch(
            ...
            category=category,
            days_to_expiry=days_to_expiry,
            historical_velocity=historical_data.get("velocity_stats")
            if historical_data
            else None,
        ...
        enhanced_result = {
            ...
            "seasonal_adjustment": composite_score / base_composite
            if base_composite > 0
            else 1.0,
    ...
    def _determine_urgency_level(
        self, days_to_expiry: int, composite_score: float
    ) -> str:
