...
class QueryAnalyzer:
    ...
    async def analyze_query_performance(
        ...
        try:
            ...
            logger.warning("Query analysis failed", query=query[:200], error=str(e))
    ...
    def _parse_explain_analyze(self, explain_json: dict[str, Any]) -> dict[str, Any]:
        ...
        analysis["rows_per_ms"] = (
            ...
            if analysis["execution_time_ms"] > 0
            else 0
        )
        ...
        analysis["buffer_hit_ratio"] = (
            ...
            if (analysis["shared_hit_blocks"] + analysis["shared_read_blocks"]) > 0
            else 1.0
        )
        ...
        if "Plans" in plan:
            analysis["child_plans"] = [
                self._analyze_child_plan(child_plan) for child_plan in plan["Plans"]
            ]
            ...
            for child in analysis["child_plans"]:
                if child["node_type"] == "Nested Loop" and child["actual_rows"] > 1000:
                    recommendations.append(
                        "Consider using hash join instead of nested loop"
                    )
...
class WriteOperationProfiler:
    ...
    async def profile_write_operation(
        ...
        query_analysis = await analyzer.analyze_query_performance(
        ...
        profile = {
            ...
            "records_per_second": record_count / (execution_time / 1000)
            if execution_time > 0
            else 0,
    ...
    async def _get_table_stats(
        ...
        try:
            ...
            logger.warning(
                "Failed to get table stats", table_name=table_name, error=str(e)
            )
            return {"error": str(e)}

    def _categorize_performance(
        self, execution_time_ms: float, record_count: int
    ) -> str:
    ...
    def get_operation_summary(self, operation_name: str) -> dict[str, Any]:
        ...
        return {
            ...
            "performance_distribution": {
                "excellent": len(
                    [p for p in profiles if p["performance_category"] == "excellent"]
                ),
                "good": len(
                    [p for p in profiles if p["performance_category"] == "good"]
                ),
                "acceptable": len(
                    [p for p in profiles if p["performance_category"] == "acceptable"]
                ),
                "slow": len(
                    [p for p in profiles if p["performance_category"] == "slow"]
                ),
                "very_slow": len(
                    [p for p in profiles if p["performance_category"] == "very_slow"]
                ),
            },
            "latest_recommendations": profiles[-1]["recommendations"]
            if profiles
            else [],
        }
...
class WriteOptimizationAdvisor:
    ...
    async def analyze_write_patterns(
        self, session: AsyncSession, store_id: str, time_window_hours: int = 24
    ) -> dict[str, Any]:
        """
    ...
    def _analyze_operation_patterns(
        ...
        return {
            "operation_types": {
                op_type: {
                    "count": len(ops),
                    "avg_execution_time_ms": sum(op["execution_time_ms"] for op in ops)
                    / len(ops),
    ...
    async def _generate_optimization_recommendations(
        self, session: AsyncSession, pattern_analysis: dict[str, Any], store_id: str
    ) -> list[dict[str, Any]]:
        ...
        if (
            "insert" in operation_types
            and operation_types["insert"]["avg_execution_time_ms"] > 100
        ):
            recommendations.append(
                {
        ...
        if (
            "update" in operation_types
            and operation_types["update"]["avg_execution_time_ms"] > 150
        ):
            recommendations.append(
                {
        ...
        if (
            peak_hours and peak_hours[0][1] > total_operations * 0.3
        ):  # >30% in peak hour
            recommendations.append(
                {
    ...
    def _summarize_performance(
        ...
        return {
            ...
            "95th_percentile_execution_time_ms": sorted(execution_times)[
...
class WritePerformanceMonitor:
    ...
    async def get_store_optimization_report(
        self, store_id: str, time_window_hours: int = 24
    ) -> dict[str, Any]:
        """
    ...
    async def analyze_slow_queries(self) -> dict[str, Any]:
        ...
        for pattern, operations in query_patterns.items():
            avg_time = sum(op["execution_time_ms"] for op in operations) / len(
                operations
            )
            analysis[pattern] = {
                ...
                "sample_recommendations": operations[0]["recommendations"][
                    :3
