...
from .barcode_detection_service import (
...
class OCRCoordinationService:
    ...
    async def _classify_single_block(
        self, text_block: str, bounding_box: dict | None, preferred_region: str
    ...
    def _resolve_classification_conflicts(
        ...
        if (
            date_score > barcode_score
            and date_score > self.thresholds["low_confidence"]
        ):
            content_type = "date"
            ...
            barcode_score > date_score
            and barcode_score > self.thresholds["low_confidence"]
        ):
            ...
            if abs(date_score - barcode_score) < 0.2:
                ...
                confidence = (
                    max(date_score, barcode_score) * 0.8
    ...
    def _calculate_date_score(
        self, text_block: str, date_results: list[DateExtractionResult]
    ) -> float:
        ...
        final_score = min(
            (base_score + context_boost) * self.priority_weights["date_context"], 1.0
        )
        return final_score

    def _calculate_barcode_score(
        self, text_block: str, barcode_results: list[BarcodeDetectionResult]
    ) -> float:
        ...
        final_score = min(
            (base_score + context_boost) * self.priority_weights["barcode_context"], 1.0
        )
        return max(final_score, 0.0)  # Ensure non-negative

    def get_classification_summary(
        self, classifications: list[OCRClassificationResult]
    ) -> dict[str, any]:
        ...
        return {
            ...
            "avg_confidence": sum(r.confidence for r in classifications) / total
            if total > 0
            else 0,
    ...
    def debug_classification_conflicts(
        self, text_block: str, classification: OCRClassificationResult
    ) -> dict[str, any]:
