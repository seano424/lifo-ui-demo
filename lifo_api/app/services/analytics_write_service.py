...
class AnalyticsWriteService:
            ...
            async with TransactionManager(session) as tx:
                try:
                    ...
                    if update_actions:
                        results[
                            "actions_updated"
                        ] = await self._bulk_update_batch_actions(
                    ...
                    if include_recommendations:
                        ...
                        ] = await self._create_urgency_recommendations(
                            session, store_id, scoring_results, tx
                        )

                    # Calculate analytics
                    results["high_urgency_items"] = high_urgency_count
                    results["avg_urgency_score"] = (
                        total_urgency / len(scoring_results) if scoring_results else 0.0
                    )
                    ...
                    logger.info(
                        "Bulk scoring results written", store_id=store_id, **results
                    )
    ...
    async def batch_analytics_events(
        self, events: list[dict[str, Any]], flush_interval_seconds: int = 5
    ) -> dict[str, Any]:
        """
    ...
    async def track_user_actions_bulk(
        self, user_id: str, store_id: str, actions: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """
            ...
            async with TransactionManager(session) as tx:
                try:
                    ...
                    for action_type, action_list in action_groups.items():
                        ...
                        results["total_quantity_impact"] += group_result[
                            "quantity_impact"
                        ]
    ...
    async def _process_event_group(
        self, event_group: str, events: list[dict[str, Any]]
    ):
            ...
            try:
                ...
                action_record = BatchAction(
                    ...
                    action_type=action.get(
                        "actual_action", action.get("action_type", action_type)
                    ),
                    ...
                    total_original_value=Decimal(
                        str(
                            action.get(
                                "original_value", action.get("total_original_value", 0)
                            )
                        )
                    ),
                    total_recovered_value=Decimal(
                        str(
                            action.get(
                                "recovered_value",
                                action.get("total_recovered_value", 0),
                            )
                        )
                    ),
                    batch_initial_quantity=Decimal(
                        str(action.get("batch_initial_quantity", 0))
                    ),
                ...
                logger.warning(
                    "Failed to track user action", action=action, error=str(e)
                )
