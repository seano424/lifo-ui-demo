...
class MCPCSVProcessor:
    ...
    async def process_csv_with_mcp(
        ...
        try:
            self.logger.info("Starting MCP CSV processing with unified service",
            ...
            final_result = {
                ...
                "processing_id": processing_id,
                "csv_validation": {
                    "total_rows": validation_result.get("processing_stats", {}).get("total_rows", 0),
                    "valid_rows": validation_result.get("processing_stats", {}).get("processed_rows", 0),
                    "warnings": validation_result.get("warnings", [])[:10],  # Limit for response size
                ...
                "performance": {
                    ...
                    "memory_usage_mb": round(self.timer.metrics.memory_usage_mb, 2) if self.timer.metrics.memory_usage_mb else None
                },
            ...
            self.logger.info("MCP CSV processing completed successfully using unified service - Performance Detail",
                           processing_id=processing_id,
                           store_id=store_id,
                           # Business metrics
                           ...
                           memory_usage_mb=round(self.timer.metrics.memory_usage_mb, 2) if self.timer.metrics.memory_usage_mb else None,
                           # Timing breakdown
                           csv_validation_ms=round(self.timer.get_timing("csv_validation") or 0, 2),
                           mcp_processing_ms=round(self.timer.get_timing("mcp_processing") or 0, 2),
                           store_verification_ms=round(self.timer.get_timing("store_verification") or 0, 2),
                           data_preparation_ms=round(self.timer.get_timing("data_preparation") or 0, 2),
    ...
    def _generate_batch_number(self, sku: str, expiry_date: str | None) -> str:
        ...
        if expiry_date:
            try:
                ...
                self.logger.debug("Failed to parse expiry date for batch number",
                                expiry_date=expiry_date, error=str(e))
