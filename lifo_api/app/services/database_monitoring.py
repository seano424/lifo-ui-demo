...
class DatabaseMonitor:
            ...
            async with self.db_manager.session_factory() as session:
                ...
                return {
                    ...
                    "avg_query_time_seconds": round(
                        stats.avg_query_time_seconds or 0, 2
                    ),
                    "status": self._assess_connection_health(
                        utilization_pct, stats.longest_query_seconds or 0
                    ),
    ...
    async def get_query_performance_stats(self) -> dict[str, Any]:
        ...
        try:
            async with self.db_manager.session_factory() as session:
                ...
                active_queries = await session.execute(
                    text("""
                    SELECT
                        pid,
                        state,
                ...
                return {
                    ...
                    "long_running_query_count": len(
                        [q for q in active_query_list if q["duration_seconds"] > 30]
                    ),
                    "timestamp": datetime.utcnow().isoformat(),
                }
        ...
        try:
            async with self.db_manager.session_factory() as session:
                ...
                if is_replica_bool:
                    ...
                    lag_query = await session.execute(
                        text("""
                        SELECT
                            CASE
                    ...
                    return {
                        ...
                        "last_replay_time": lag_info.last_replay_time.isoformat()
                        if lag_info.last_replay_time
                        else None,
                        "status": "healthy"
                        if (lag_info.lag_seconds or 0)
                        < self.thresholds["replication_lag_warning_sec"]
                        else "warning",
                        "timestamp": datetime.utcnow().isoformat(),
                    }
                else:
                    # This is primary - check for replicas
                    replica_stats = await session.execute(
                        text("""
                        SELECT
                            client_addr,
                            state,
                        ...
                        replicas.append(
                            {
                                ...
                                "state": row.state,
                                "flush_lag_mb": round(
                                    (row.flush_lag_bytes or 0) / (1024 * 1024), 2
                                ),
                                "replay_lag_mb": round(
                                    (row.replay_lag_bytes or 0) / (1024 * 1024), 2
                                ),
                            }
                        )
    ...
    async def get_lock_stats(self) -> dict[str, Any]:
        ...
        try:
            async with self.db_manager.session_factory() as session:
                ...
                deadlock_stats = await session.execute(
                    text("""
                    SELECT
                ...
                return {
                    ...
                    "total_deadlocks": deadlock_row.total_deadlocks
                    if deadlock_row
                    else 0,
    ...
    async def get_cache_stats(self) -> dict[str, Any]:
        ...
        try:
            async with self.db_manager.session_factory() as session:
                ...
                buffer_stats = await session.execute(
                    text("""
                    SELECT
                ...
                return {
                    ...
                    "shared_buffers": f"{buffer_row.setting}{buffer_row.unit}"
                    if buffer_row
                    else "unknown",
        ...
        try:
            async with self.db_manager.session_factory() as session:
                ...
                try:
                    ...
                    self.logger.warning(
                        "CSV upload logging table not available", error=str(e)
                    )
                ...
                return {
                    ...
                    "data_health": {
                        "expired_batch_ratio": round(
                            count_row.expired_batch_count
                            / max(count_row.active_batch_count, 1),
                            3,
                        ),
                        "scoring_coverage": round(
                            count_row.recent_scores_count
                            / max(count_row.active_batch_count, 1),
                            3,
                        ),
                    },
    ...
    def _assess_connection_health(
        self, utilization_pct: float, longest_query_sec: float
    ) -> str:
        """Assess connection health based on utilization and query times"""
        if (
            utilization_pct > self.thresholds["connection_count_critical"]
            or longest_query_sec > self.thresholds["query_time_critical_ms"] / 1000
        ):
            return "critical"
        elif (
            utilization_pct > self.thresholds["connection_count_warning"]
            or longest_query_sec > self.thresholds["query_time_warning_ms"] / 1000
        ):
            return "warning"
        else:
            return "healthy"
    ...
    async def get_comprehensive_health_check(self) -> dict[str, Any]:
        ...
        return {
            ...
            "checks": {
                ...
                else {"error": str(connection_stats)},
                "query_performance": query_stats
                if isinstance(query_stats, dict)
                else {"error": str(query_stats)},
                "table_sizes": table_stats
                if isinstance(table_stats, dict)
                else {"error": str(table_stats)},
                "replication": replication_stats
                if isinstance(replication_stats, dict)
                else {"error": str(replication_stats)},
                "locks": lock_stats
                if isinstance(lock_stats, dict)
                else {"error": str(lock_stats)},
                "cache": cache_stats
                if isinstance(cache_stats, dict)
                else {"error": str(cache_stats)},
                "lifo_metrics": lifo_stats
                if isinstance(lifo_stats, dict)
                else {"error": str(lifo_stats)},
            },
            "recommendations": self._generate_recommendations(statuses, health_checks),
        }

    def _generate_recommendations(
        self, statuses: list[str], health_data: list[Any]
    ) -> list[str]:
        ...
        cache_data = (
            health_data[5]
            if len(health_data) > 5 and isinstance(health_data[5], dict)
            else {}
        )
        ...
        table_data = (
            health_data[2]
            if len(health_data) > 2 and isinstance(health_data[2], dict)
            else {}
        )
