...
class CSVTemplateGenerator:
    ...
    def generate_custom_template(
        self, columns: list[str], include_samples: bool = True, sample_count: int = 3
    ) -> str:
        """
    ...
    def _generate_csv_content(
        self, columns: list[str], data: list[dict[str, str]]
    ) -> str:
        """
        ...
        self.logger.info(
            "CSV template generated", columns=len(columns), sample_rows=len(data)
        )
    ...
    def _get_category_specific_sample_data(
    ...
    def _get_custom_sample_data(
        self, columns: list[str], sample_count: int
    ) -> list[dict[str, str]]:
