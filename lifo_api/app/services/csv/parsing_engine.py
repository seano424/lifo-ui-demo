...
class CSVParsingEngine:
    ...
    async def parse_csv_content(
        self, content: bytes = None, file_path: str = None, encoding: str = "utf-8"
    ) -> dict[str, Any]:
        """
    ...
    def _decode_csv_content(
        ...
        if content:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=404, detail=f"CSV file not found: {file_path}"
                )
    ...
    async def _parse_csv_structure(self, csv_content: str) -> dict[str, Any]:
        ...
        try:
            ...
            for row_idx, row in enumerate(csv_reader):
                ...
                # Sanitize each cell
                sanitized_row = [
                    self.security_validator.sanitize_csv_cell(cell) for cell in row
                ]
            ...
            self.logger.warning(
                "CSV module parsing failed, trying pandas", error=str(e)
            )
    ...
    async def _parse_with_pandas_fallback(self, csv_content: str) -> dict[str, Any]:
        ...
        try:
            ...
            for row in data_rows:
                sanitized_row = [
                    self.security_validator.sanitize_csv_cell(str(cell)) for cell in row
                ]
            ...
            raise HTTPException(
                status_code=400, detail=f"CSV parsing failed: {str(e)}"
            ) from e
    ...
    def convert_to_dataframe(self, parsed_data: dict[str, Any]) -> pd.DataFrame:
        ...
        try:
            df = pd.DataFrame(
                data=parsed_data["data_rows"], columns=parsed_data["headers"]
            )
            ...
            self.logger.info(
                "CSV converted to DataFrame", rows=len(df), columns=len(df.columns)
            )
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to convert CSV to DataFrame: {str(e)}"
            ) from e

    def parse_date_field(
        self, date_value: Any, field_name: str = "date"
    ) -> date | None:
        """
