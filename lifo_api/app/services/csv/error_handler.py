...
class CSVErrorHandler:
    ...
    def format_errors_for_response(self, limit: int = 50) -> list[dict[str, Any]]:
        ...
        if remaining_limit > 0:
            other_errors = [e for e in self.errors if e.severity != ErrorSeverity.CRITICAL]
    ...
    def raise_if_critical(self) -> None:
        ...
        if self.has_critical_errors():
            ...
            if len(critical_errors) > 3:
                combined_message += f" (and {len(critical_errors) - 3} more critical errors)"
    ...
    def get_performance_metrics(self) -> dict[str, Any]:
        ...
        return {
            "error_rate": (len(self.errors) / total_rows * 100) if total_rows > 0 else 0,
            "warning_rate": (len(self.warnings) / total_rows * 100) if total_rows > 0 else 0,
            "success_rate": (self.processing_stats["processed_rows"] / total_rows * 100) if total_rows > 0 else 0,
            "total_issues": total_issues,
            "critical_issues": sum(1 for e in self.errors if e.severity == ErrorSeverity.CRITICAL),
            "rows_per_error": total_rows / len(self.errors) if len(self.errors) > 0 else total_rows
        }
