...
class UnifiedCSVService:
    ...
    def __init__(self, store_id: str, user_id: str):
        ...
        self.logger = logger.bind(
            component="unified_csv_service", store_id=store_id, user_id=user_id
        )
    ...
    async def process_csv_upload(
        ...
        """
        processing_id = (
            f"csv_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:8]}"
        )
        ...
        for idx, row in df.iterrows():
            ...
            for price_field in ["cost_price", "selling_price"]:
                if price_field in df.columns:
                    ...
                    if (
                        price is None
                        and not pd.isna(row.get(price_field))
                        and str(row.get(price_field)).strip()
                    ):
                        self.error_handler.add_validation_error(
                            field=price_field,
    ...
    async def _process_for_import(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                self.error_handler.add_error(
                    message=str(e), error_type=ErrorType.SYSTEM, row_number=row_number
                )
    ...
    async def _process_single_row(
        ...
        processed["category_id"] = await self.category_mapper.resolve_category_uuid(
            category_code
        )

        # Numeric fields
        processed["quantity"] = (
            self.parsing_engine.parse_numeric_field(
    ...
    async def _validate_business_rules(
        self, data: dict[str, Any], row_number: int
    ) -> None:
    ...
    async def _convert_to_batch_requests(
        ...
        for item in processed_data:
            ...
            if not item.get("batch_number"):
                item["batch_number"] = self._generate_batch_number(
        ...
        if "category" in df.columns:
            ...
            for cat in unique_categories:
                if not self.category_mapper.validate_category_code(cat):
                    suggestions[cat] = self.category_mapper.get_category_suggestions(
                        cat
                    )
    ...
    async def _generate_optimization_tips(
        ...
        missing_batch_numbers = sum(
            1 for item in processed_data if not item.get("batch_number")
        )
        ...
        missing_suppliers = sum(
            1 for item in processed_data if not item.get("supplier")
        )
    ...
    def generate_category_template(self, category_code: str) -> str:
        ...
        return self.template_generator.generate_category_specific_template(
            category_code
        )
