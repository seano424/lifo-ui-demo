...
class UnifiedWriteService:
    ...
    async def bulk_inventory_operations(
        ...
        logger.info(
            ...
            **{
                ...
                if k not in ["successful_operations", "failed_operations"]
            },
        )
                ...
                try:
                    ...
                    if create_recommendations:
                        recommendations_created = (
                            await self._create_scoring_recommendations(
                                session, store_id, scoring_results, tx
                            )
                        )
            ...
            async with TransactionManager(session) as tx:
                try:
                    ...
                    if "user_actions" in mobile_data:
                        results[
                            "user_actions_synced"
                        ] = await self._sync_mobile_actions(
                    ...
                    if "analytics_events" in mobile_data:
                        results[
                            "analytics_events_recorded"
                        ] = await self._record_mobile_analytics(
                            session,
                            user_id,
                            store_id,
                            mobile_data["analytics_events"],
                            tx,
                        )
    ...
    async def _create_batch_record(
        ...
        return {"batch_id": batch.batch_id, "batch_number": batch_number}
                ...
                update_query = (
                    ...
                    .where(
                        and_(
                            ...
                            == "maintain",  # Only update pending actions
                        )
                    )
                    .values(
            ...
            try:
                if result["urgency_score"] >= 0.6:  # Only create for high urgency
                    ...
                    if not existing_check.scalar_one_or_none():
                        ...
                        batch_result = await session.execute(
                            select(Batch).where(
                                Batch.batch_id == uuid.UUID(result["batch_id"])
                            )
                        )
                        batch = batch_result.scalar_one_or_none()
    ...
    async def _sync_mobile_actions(
        ...
        for action_data in mobile_actions:
            try:
                ...
                batch_result = await session.execute(
                    select(Batch).where(
                        Batch.batch_id == uuid.UUID(action_data["batch_id"])
                    )
                )
                batch = batch_result.scalar_one_or_none()
                ...
                if not batch:
                    logger.warning(
                        "Batch not found for mobile action",
                ...
                action = BatchAction(
                    ...
                    action_type=action_data.get(
                        "actual_action", action_data.get("action_type", "maintain")
                    ),
                    ...
                    total_original_value=Decimal(
                        str(
                            action_data.get(
                                "original_value",
                                action_data.get("total_original_value", 0),
                            )
                        )
                    ),
                    total_recovered_value=Decimal(
                        str(
                            action_data.get(
                                "recovered_value",
                                action_data.get("total_recovered_value", 0),
                            )
                        )
                    ),
                    batch_initial_quantity=batch.initial_quantity,
        ...
        for update_data in batch_updates:
            try:
                ...
                if current_batch:
                    ...
                    mobile_last_sync = datetime.fromisoformat(
                        update_data.get("last_sync", "2000-01-01T00:00:00")
                    )
                    ...
                    if "current_quantity" in update_data:
                        current_batch.current_quantity = Decimal(
                            str(update_data["current_quantity"])
                        )
                ...
                logger.warning(
                    "Failed to sync batch update", update_data=update_data, error=str(e)
                )
    ...
    async def _handle_single_batch_update(
        ...
        return {"batch_id": batch_id, "updated_fields": list(update_data.keys())}
