...
class WriteCache:
    ...
    def invalidate(self, key: str = None, prefix: str = None) -> int:
        ...
        if key:
            ...
            keys_to_remove = [
                k for k in self.cache.keys() if k.startswith(f"{prefix}:")
            ]
...
class WriteBehindProcessor:
    ...
    async def queue_write(self, operation: dict[str, Any]) -> str:
        ...
        operation.update({"operation_id": operation_id, "queued_at": datetime.utcnow()})

        self.write_queue.append(operation)

        # Start processing if not already running
        if not self.processing and (
            len(self.write_queue) >= self.batch_size or self._has_old_operations()
        ):
    ...
    async def _process_write_batch(self, batch: list[dict[str, Any]]):
        ...
        async with async_session()() as session:
            try:
                ...
                logger.error(
                    "Write-behind batch failed", batch_size=len(batch), error=str(e)
                )
                raise

    async def _process_operation_group(
        self, session: AsyncSession, operations: list[dict[str, Any]]
    ):
        ...
        if operation_type == "bulk_insert":
            ...
            logger.warning(
                "Unknown write-behind operation type", operation_type=operation_type
            )

    async def _process_bulk_inserts(
        self, session: AsyncSession, table_name: str, operations: list[dict[str, Any]]
    ):
        ...
        if all_records:
            ...
            await bulk_optimizer.bulk_insert_ignore_duplicates(table_name, all_records)
...
class WriteCacheOptimizer:
    ...
    async def get_cached_products(
        self, store_id: str, barcodes: list[str]
    ...
    async def cache_products(self, store_id: str, products: dict[str, str]) -> None:
    ...
    async def get_cached_batch_data(
        self, store_id: str, batch_ids: list[str]
    ...
    async def invalidate_batch_cache(
        self, store_id: str, batch_ids: list[str] = None
    ) -> int:
    ...
    async def cache_store_metadata(
        self, store_id: str, metadata: dict[str, Any]
    ) -> None:
        ...
        async with self.session_factory() as session:
            try:
                ...
                result = await session.execute(
                    frequent_products_query, {"store_id": store_id}
                )
                ...
                logger.error(
                    "Product cache warming failed", store_id=store_id, error=str(e)
                )
                return {"status": "failed", "error": str(e)}

    async def schedule_write_behind(
        self, operation_type: str, table_name: str, data: dict[str, Any]
    ) -> str:
        """
        Schedule a write-behind operation for batch processing
        """
        operation = {"operation_type": operation_type, "table_name": table_name, **data}

        return await self.write_behind_processor.queue_write(operation)

    async def optimize_write_caching(
        self, operation_type: str, store_id: str, data: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        if operation_type == "batch_creation":
            ...
            if barcodes:
                products, missing_barcodes = await self.get_cached_products(
                    store_id, barcodes
                )
