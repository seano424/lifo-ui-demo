...
class DateExtractionService:
    ...
    async def extract_dates_from_text_blocks(
        ...
        validated_results.sort(
            key=lambda x: (self._get_date_type_priority(x.date_type), x.confidence),
            reverse=True,
        )
        ...
        for i, result in enumerate(final_results):
            logger.debug(
                f"  Result {i + 1}: {result.date} ({result.date_type}) confidence={result.confidence}"
            )
        ...
        for pattern_name in pattern_priority:
            ...
            for match in matches:
                ...
                if result and result.confidence >= self.confidence_thresholds["low"]:
                    logger.debug(
                        f"Pattern {pattern_name} result accepted: {result.date} (confidence: {result.confidence})"
                    )
                    results.append(result)
                else:
                    if result:
                        logger.debug(
                            f"Pattern {pattern_name} result rejected: confidence {result.confidence} < {self.confidence_thresholds['low']}"
                        )
                    else:
    ...
    def _create_date_result(
        ...
        try:
            ...
            parsed_date = self._parse_date_groups(
                groups,
                pattern_info_with_context,
                detected_language,
                context_text,
                pattern_name,
            )
            if not parsed_date:
                ...
                logger.debug(
                    f"Pattern {pattern_name} successfully parsed date: {parsed_date}"
                )

            # Determine date type from context
            date_type = self._classify_date_type(
                context_text, pattern_info.get("type", "unknown")
            )
    ...
    def _parse_date_groups(
        ...
        try:
            ...
            logger.debug(
                f"Attempting to parse with format: {format_type}, groups: {groups}"
            )
            ...
            if (
                format_type in ["YYYYMMDD", "DDMMYYYY", "DDMMYY"]
                and len(filtered_groups) < 1
            ):
                return None
            elif (
                format_type not in ["YYYYMMDD", "DDMMYYYY", "DDMMYY"]
                and len(filtered_groups) < 3
            ):
                return None

            # Smart format detection for context-aware patterns
            if format_type == "SMART":
                return self._parse_smart_date(
                    filtered_groups, pattern_info, detected_language
                )
                ...
                if len(filtered_groups[0]) == 4:
                    year, month, day = (
                        ...
                        int(filtered_groups[1]),
                        int(filtered_groups[2]),
                    )
                else:
                    day, month, year = (
                        int(filtered_groups[0]),
                        int(filtered_groups[1]),
                        self._normalize_year(int(filtered_groups[2])),
                    )
                ...
                return self._parse_month_name_date(
                    filtered_groups, format_type, detected_language
                )
                ...
                month_num: int | None = self._get_month_number(
                    month_str.lower(), detected_language
                )
        ...
        try:
            ...
            # If third value is 4 digits, it's likely DD/MM/YYYY or MM/DD/YYYY
                ...
                if first <= 12 and second > 12:
                    ...
                    has_us_indicator = any(
                        indicator in context_text.lower() for indicator in us_indicators
                    )
                    has_eu_indicator = any(
                        indicator in context_text.lower() for indicator in eu_indicators
                    )
    ...
    def _parse_month_name_date(
        self, groups: tuple, format_type: str, detected_language: str | None
    ) -> datetime | None:
    ...
    def _parse_compact_date_yyyy(self, date_str: str) -> datetime | None:
        ...
        try:
            logger.debug(
                f"Parsing YYYY compact date: '{date_str}' (length: {len(date_str)})"
            )
            if len(date_str) == 8:  # YYYYMMDD
                ...
                logger.debug(
                    f"Parsed YYYY date components: year={year}, month={month}, day={day} -> {parsed_date}"
                )
                return parsed_date
            else:
    ...
    def _get_month_number(
        self, month_str: str, detected_language: str | None
    ) -> int | None:
    ...
    def _calculate_confidence(
        ...
        priority_boost = {"very_high": 0.4, "high": 0.3, "medium": 0.2, "low": 0.1}
    ...
    def _has_consistent_format(self, text: str) -> bool:
        ...
        return (
            not re.search(r"\d{1}\d{2}", text)  # Avoid mixed digit counts
            and not re.search(
                r"[a-zA-Z]\d", text
            )  # No letters directly touching numbers
            and len(text.strip()) >= 6  # Minimum reasonable length
        )
    ...
    def _remove_duplicate_dates(
        self, results: list[DateExtractionResult]
    ) -> list[DateExtractionResult]:
