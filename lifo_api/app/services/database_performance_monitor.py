    ...
    async def optimize_write_operations(
        ...
        try:
            ...
            for target in optimization_targets:
                try:
                    optimization = await self._apply_optimization(target, baseline_metrics)
                    if optimization["applied"]:
                        optimization_results["optimizations_applied"].append(optimization)
                    else:
                        optimization_results["recommendations_remaining"].append(optimization)
            ...
            logger.error(
                "Write operation optimization failed",
                error=str(e)
            )
            raise
    ...
    async def _monitor_database_performance(self):
        ...
        while self._monitoring_active:
            try:
                ...
                logger.error(
                    "Database performance monitoring error",
                    error=str(e)
                )
            ...
            analysis["most_problematic_queries"] = [
                {
                    "pattern": pattern,
                    "count": len(queries),
                    "avg_time_ms": sum(q.get("execution_time_ms", 0) for q in queries) / len(queries)
                }
