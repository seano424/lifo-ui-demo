...
async def get_global_products(
    ...
    search: Optional[str] = Query(
        None, description="Search term for name/brand/barcode"
    ),
...
async def get_global_product(
    ...
    try:
        ...
        logger.error(
            "Error fetching global product", product_id=product_id, error=str(e)
        )
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def search_by_barcode(
    ...
    try:
        ...
        logger.error(
            "Error searching product by barcode", barcode=barcode, error=str(e)
        )
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_store_products(
    ...
    try:
        ...
        query = (
            select(StoreProduct, GlobalProduct)
            .join(GlobalProduct, StoreProduct.product_id == GlobalProduct.product_id)
        ...
        for store_product, global_product in store_products:
            product_dict = {
                ...
                "default_cost_price": float(store_product.default_cost_price)
                if store_product.default_cost_price
                else None,
                "default_selling_price": float(store_product.default_selling_price)
                if store_product.default_selling_price
                else None,
                ...
                "added_at": store_product.created_at.isoformat()
                if store_product.created_at
                else None,
            }
            products_data.append(product_dict)
