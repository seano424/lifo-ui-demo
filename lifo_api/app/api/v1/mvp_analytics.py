...
from app.models.scan_models import BatchInsights, MVPMetrics
...
async def get_mvp_metrics(
    ...
    try:
        ...
        processing_time_ms = (time.time() - start_time) * 1000
        logger.error(
            "MVP metrics calculation failed",
            store_id=store_id,
            error=str(e),
            processing_time_ms=processing_time_ms,
            user_id=current_user["sub"],
...
async def get_batch_insights(
    ...
    try:
        ...
        insights = await _generate_batch_insights(
            inventory_data, analysis_depth, store_id, read_ops
        )

        processing_time_ms = (time.time() - start_time) * 1000

        logger.info(
            "Batch insights generated",
            store_id=store_id,
        ...
        processing_time_ms = (time.time() - start_time) * 1000
        logger.error(
            "Batch insights generation failed",
            store_id=store_id,
            error=str(e),
            processing_time_ms=processing_time_ms,
            user_id=current_user["sub"],
...
async def get_scan_workflow_stats(
    ...
    try:
        ...
        logger.info(
            "Scan workflow stats calculated",
            store_id=store_id,
            days=days,
            processing_time_ms=processing_time_ms,
            user_id=current_user["sub"],
...
async def get_waste_prevention_impact(
    ...
    try:
        ...
        impact_analysis = await _calculate_waste_prevention_impact(
            store_id, comparison_period, baseline_period, read_ops
        )

        processing_time_ms = (time.time() - start_time) * 1000

        logger.info(
            "Waste prevention impact calculated",
            store_id=store_id,
        ...
        processing_time_ms = (time.time() - start_time) * 1000
        logger.error(
            "Waste prevention impact calculation failed",
            store_id=store_id,
            error=str(e),
            processing_time_ms=processing_time_ms,
            user_id=current_user["sub"],
...
async def get_action_effectiveness(
    ...
    try:
        ...
        effectiveness_analysis = await _calculate_action_effectiveness(
            store_id, action_type, days, read_ops
        )

        processing_time_ms = (time.time() - start_time) * 1000

        logger.info(
            "Action effectiveness calculated",
            store_id=store_id,
        ...
        processing_time_ms = (time.time() - start_time) * 1000
        logger.error(
            "Action effectiveness calculation failed",
            store_id=store_id,
            error=str(e),
            processing_time_ms=processing_time_ms,
            user_id=current_user["sub"],
...
async def _calculate_mvp_metrics(
    ...
    average_batch_visibility_improvement = min(
        100.0, (total_batches / max(1, total_batches - 10)) * 15.0
    )
...
async def _generate_batch_insights(
    ...
    if high_value_expiring:
        total_value_at_risk = sum(
            i["current_quantity"] * i["selling_price"] for i in high_value_expiring
        )
    ...
    high_risk_categories = [
        cat for cat, perf in category_performance.items() if perf["risk_score"] > 0.5
    ]
