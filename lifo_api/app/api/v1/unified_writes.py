...
async def create_unified_inventory_batch(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Unified batch creation failed: {str(e)}"
        )
    ...
    try:
        ...
        if len(bulk_request.operations) > 100:
            background_tasks.add_task(
                _process_bulk_analytics, store_id, result, current_user["sub"]
            )
    ...
    try:
        ...
        return {
            ...
            "insights": {
                ...
                "recommendation_coverage": result["recommendations_created"]
                / len(analytics_request.scoring_results)
                * 100,
            },
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Bulk scoring write failed: {str(e)}"
        )
...
async def mobile_data_sync(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Mobile sync failed: {str(e)}")
...
async def csv_bulk_import_optimized(
    ...
    try:
        ...
        if len(csv_data) > 500:
            background_tasks.add_task(
                _process_csv_import_analytics, store_id, result, current_user["sub"]
            )
...
async def batch_action_workflow(
    ...
    try:
        ...
        result = await enhanced_ops.batch_action_workflow(
            store_id=store_id, user_id=current_user["sub"], actions=actions
        )
        ...
        return {
            ...
            "tracking": {"action_tracking_records": result["action_tracking_records"]},
            "performance": {
                "execution_time_ms": result["execution_time_ms"],
        ...
        raise HTTPException(status_code=500, detail=f"Action workflow failed: {str(e)}")
...
async def _process_bulk_analytics(
    store_id: str, operation_result: Dict[str, Any], user_id: str
):
    ...
    try:
        ...
        logger.error(
            "Background bulk analytics failed", error=str(e), store_id=store_id
        )


async def _process_csv_import_analytics(
    store_id: str, import_result: Dict[str, Any], user_id: str
):
    ...
    try:
        ...
        logger.error("Background CSV analytics failed", error=str(e), store_id=store_id)
