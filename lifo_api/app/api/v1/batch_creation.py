    ...
    try:
        ...
        async with db.get_session() as session:
            ...
            if row.total_scan_batches > 0:
                ...
                enrichment_percentage = (row.enriched_products / row.total_scan_batches) * 100
                session_linking_percentage = (row.linked_to_sessions / row.total_scan_batches) * 100
            ...
            return {
                ...
                'avg_expiry_confidence': float(row.avg_expiry_confidence) if row.avg_expiry_confidence else None,
                ...
                'first_scan_batch': row.first_scan_batch.isoformat() if row.first_scan_batch else None,
                'latest_scan_batch': row.latest_scan_batch.isoformat() if row.latest_scan_batch else None,
                'generated_at': __import__('datetime').datetime.utcnow().isoformat()
            }
