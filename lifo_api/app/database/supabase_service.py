...
class SupabaseService:
    ...
    async def save_ocr_result(
        ...
        try:
            ...
            result = (
                user_client.schema("inventory")
                .table("ocr_processing_batches")
                .insert(batch_data)
                .execute()
            )
    ...
    async def create_batch_from_ocr(
        ...
        try:
            ...
            inventory_batch = {
                "store_id": store_id,
                "batch_name": batch_data.get(
                    "batch_name", f"OCR Batch {datetime.now().strftime('%Y%m%d_%H%M')}"
                ),
    ...
    async def check_user_store_access(self, user_token: str, store_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to check store access", error=str(e), store_id=store_id
            )
            return False
    ...
    async def get_user_stores(self, user_token: str) -> list[dict[str, Any]]:
        ...
        try:
            user_client = self.get_user_client(user_token)
            result = (
                user_client.schema("business").table("stores").select("*").execute()
            )
