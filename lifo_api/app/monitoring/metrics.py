...
class MetricsCollector:
    ...
    def __init__(self):
        ...
        self.time_series_metrics = defaultdict(lambda: deque(maxlen=1000))  # Keep last 1000 data points
    ...
    def record_api_request(
        ...
        with self.lock:
            ...
            is_mobile_endpoint = any(mobile_path in endpoint for mobile_path in [
                "/mobile-summary", "/batch-quick-score", "/store-health", "/batch-list-mobile"
            ])
            ...
            if is_mobile_endpoint:
                ...
                if response_time_ms > mobile_threshold:
                    ...
                    self._trigger_alert(
                        ...
                        {"endpoint": endpoint, "response_time_ms": response_time_ms, "threshold": mobile_threshold}
                    )
            ...
            logger.info(
                ...
                is_mobile=is_mobile_endpoint,
                mobile_target_met=not (is_mobile_endpoint and response_time_ms > mobile_threshold) if is_mobile_endpoint else None
            )
    ...
    def record_database_query(
        ...
        with self.lock:
            ...
            if success:
                ...
                metrics["error_count"] += 1
                logger.error("Database query failed", query_name=query_name, error=error)
            ...
            if execution_time_ms > self.thresholds["database_query_time_ms"]["warning"]:
                ...
                if execution_time_ms > self.thresholds["database_query_time_ms"]["critical"]:
                    self._trigger_alert(
                        "slow_database_query",
                        ...
                        {"query_name": query_name, "execution_time_ms": execution_time_ms, "result_count": result_count}
                    )
            ...
            if metrics["hits"] + metrics["misses"] > 0:
                ...
                if hit_rate < self.thresholds["cache_hit_rate_percent"]["critical"]:
                    self._trigger_alert(
                        ...
                        {"cache_name": cache_name, "hit_rate": hit_rate, "total_operations": metrics["total_operations"]}
                    )
        ...
        def monitor_system():
            while True:
                try:
                    ...
                    if memory.percent > self.thresholds["memory_usage_percent"]["critical"]:
                        self._trigger_alert(
                            "high_memory_usage",
                            f"Memory usage is {memory.percent:.1f}% (critical threshold: {self.thresholds['memory_usage_percent']['critical']}%)",
                            {"memory_percent": memory.percent, "available_mb": memory.available / (1024 * 1024)}
                        )
    ...
    def _trigger_alert(self, alert_type: str, message: str, context: Dict[str, Any]):
        ...
        with self.lock:
            ...
            if len(self.alerts_triggered[alert_type]) > 100:
                self.alerts_triggered[alert_type] = self.alerts_triggered[alert_type][-100:]
                ...
                if key.startswith("api_"):
                    ...
                    if metrics["total_requests"] > 0:
                        summary["api_metrics"][endpoint_name] = {
                            ...
                            "avg_response_time_ms": metrics["total_response_time_ms"] / metrics["total_requests"],
                            ...
                            "mobile_target_violations": metrics.get("mobile_target_violations", 0),
                    ...
                    if metrics["total_queries"] > 0:
                        summary["database_metrics"][query_name] = {
                            ...
                            "avg_execution_time_ms": metrics["total_execution_time_ms"] / metrics["total_queries"],
                            "success_rate": metrics["success_count"] / metrics["total_queries"],
                            "avg_result_count": metrics["total_results"] / metrics["total_queries"],
                            "slow_query_rate": metrics["slow_queries"] / metrics["total_queries"],
                            "min_execution_time_ms": metrics["min_execution_time_ms"] if metrics["min_execution_time_ms"] != float('inf') else 0,
                    ...
                    if metrics["total_operations"] > 0:
                        hit_rate = (metrics["hits"] / (metrics["hits"] + metrics["misses"])) * 100 if (metrics["hits"] + metrics["misses"]) > 0 else 0
    ...
    def _calculate_performance_health(self) -> Dict[str, Any]:
        ...
        if total_mobile_requests > 0:
            ...
            mobile_health = max(0, 1 - mobile_violation_rate * 2)  # Penalty for violations
            health_scores.append(mobile_health)
            
            if mobile_violation_rate > 0.1:  # >10% violations
                issues.append(f"Mobile performance violations: {mobile_violation_rate:.1%} of requests")
        ...
        if "memory_usage_percent" in system_metrics:
            ...
            if system_metrics["memory_usage_percent"] > 80:
                issues.append(f"High memory usage: {system_metrics['memory_usage_percent']:.1f}%")
        ...
        return {
            ...
            "component_scores": {
                "mobile_performance": health_scores[0] if len(health_scores) > 0 else 1.0,
                "system_resources": health_scores[1] if len(health_scores) > 1 else 1.0,
                "cache_performance": health_scores[2] if len(health_scores) > 2 else 1.0
            },
            "issues": issues,
    ...
    def _get_health_recommendations(self, issues: List[str]) -> List[str]:
        ...
        for issue in issues:
            if "mobile performance" in issue.lower():
                recommendations.append("Optimize mobile queries or increase server resources")
