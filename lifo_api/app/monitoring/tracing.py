    ...
    def _setup_exporters(self) -> None:
        ...
        if otlp_endpoint:
            otlp_exporter = OTLPSpanExporter(
                endpoint=otlp_endpoint, headers=getattr(settings, "otlp_headers", {})
            )
    ...
    def cleanup(self) -> None:
        ...
        for instrumentor_name in self.instrumentors:
            try:
                ...
                logger.warning(
                    f"Failed to uninstrument {instrumentor_name}", error=str(e)
                )
...
def trace_database_operation(operation_name: str):
    ...
    return distributed_tracing.trace_business_operation(operation_name, "database")


def trace_api_operation(operation_name: str):
    """Decorator for tracing API operations"""
    return distributed_tracing.trace_business_operation(operation_name, "api")
...
def trace_scoring_operation(operation_name: str, category: str | None = None):
    ...
    return distributed_tracing.trace_business_operation(
        operation_name, "scoring", category=category
    )
...
def trace_batch_operation(operation_name: str, batch_id: str | None = None):
    ...
    return distributed_tracing.trace_business_operation(
        operation_name, "batch", batch_id=batch_id
    )
