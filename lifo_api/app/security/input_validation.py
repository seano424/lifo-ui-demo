...
class SecurityInputValidator:
    ...
    def _validate_sql_input(self, value: str, strict_mode: bool = True):
        ...
        for pattern in self.compiled_patterns["sql"]:
            if pattern.search(value):
                logger.warning("SQL injection attempt detected", pattern=pattern.pattern, value=value[:50])
    ...
    def _validate_against_injection_attacks(self, value: str):
        ...
        for pattern in self.compiled_patterns["xss"]:
            if pattern.search(value):
                logger.warning("XSS attempt detected", pattern=pattern.pattern, value=value[:50])
                raise InputValidationError("Input contains potentially dangerous script content")
        
        # Check for command injection patterns
        for pattern in self.compiled_patterns["cmd"]:
            if pattern.search(value):
                logger.warning("Command injection attempt detected", pattern=pattern.pattern, value=value[:50])
                raise InputValidationError("Input contains potentially dangerous command patterns")
        
        # Check for path traversal patterns
        for pattern in self.compiled_patterns["path"]:
            if pattern.search(value):
                logger.warning("Path traversal attempt detected", pattern=pattern.pattern, value=value[:50])
                raise InputValidationError("Input contains potentially dangerous path patterns")
    ...
    def _validate_url(self, url: str) -> str:
        ...
        if parsed_url.hostname:
            if self._is_private_ip(parsed_url.hostname):
                logger.warning("Private IP access attempt", hostname=parsed_url.hostname)
    ...
    def _validate_alphanumeric(self, value: str) -> str:
        ...
        if not re.match(r'^[a-zA-Z0-9_\-\.]+$', value):
            raise InputValidationError("Input must contain only alphanumeric characters, hyphens, underscores, and dots")
        return value
