...
from app.security.input_validation import security_validator, InputValidationError
from app.security.rate_limiting import get_rate_limiter
from app.security.security_monitor import get_security_monitor, SecurityEventType
...
class ComprehensiveSecurityMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app):
        ...
        self.high_security_endpoints = {
            "/api/v1/csv", "/api/v1/upload", "/api/v1/analytics",
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if self.security_monitor.detect_security_scan(request):
                logger.warning("Security scanner detected", ip=client_ip, path=request.url.path)
            ...
            rate_limit_allowed, rate_limit_reason = self.rate_limiter.check_rate_limit(request)
            ...
            if self.rate_limiter.detect_ddos_attack(request):
                ...
                return self._create_security_response(
                    "DDoS attack detected - IP temporarily banned",
                    status_code=429
                )
            ...
            if request.url.query:
                try:
                    ...
                    self.security_monitor.record_security_event(
                        ...
                        request,
                        severity="high",
                        details={"validation_error": str(e), "input_type": "query_params"}
                    )
                    return self._create_security_response(
            ...
            if self.is_high_security_endpoint(request.url.path):
                high_security_valid = await self._validate_high_security_request(request)
                if not high_security_valid:
                    return self._create_security_response(
                        "High-security endpoint validation failed",
                        status_code=403
                    )
        ...
        for header in suspicious_headers:
            if header in request.headers:
                ...
                try:
                    ...
                    validation_result["valid"] = False
                    validation_result["details"][f"suspicious_header_{header}"] = value[:50]
