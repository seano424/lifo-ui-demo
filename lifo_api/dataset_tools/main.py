...
async def download_foodiml(
    config: DatasetConfig, sample_size: int = 1000
) -> Dict[str, Any]:
...
async def analyze_dataset(
    config: DatasetConfig, dataset_path: Path, create_validation: bool = True
) -> Dict[str, Any]:
    ...
    logger.info("Dataset analysis completed. Report saved to analysis directory.")
    return analysis


async def create_ocr_validation_suite(
    config: DatasetConfig, dataset_path: Path, max_test_cases: int = 200
) -> Dict[str, Any]:
    ...
    with ProgressTracker() as progress:
        suite = await validator.create_validation_suite(
            dataset_path, max_test_cases=max_test_cases, progress_tracker=progress
        )
    ...
    logger.info(
        f"OCR validation suite created with {suite.total_test_cases} test cases"
    )
...
def _print_pipeline_summary(results: Dict[str, Any]) -> None:
    ...
    if "openfoodfacts" in results:
        ...
        if "error" not in off_results:
            print(
                f"✓ Open Food Facts: {off_results.get('products_downloaded', 0)} products, "
    ...
    if "foodiml" in results:
        ...
        if "error" not in foodiml_results:
            ...
            print(
                f"✓ FooDI-ML: {download_stats.get('downloaded', 0)} images downloaded"
            )
        else:
    ...
    if "analysis" in results:
        for dataset, analysis in results["analysis"].items():
            if "error" not in analysis:
                ...
                print(
                    f"✓ {dataset.title()} Analysis: {stats.get('total_images', 0)} images analyzed, "
    ...
    if "ocr_validation" in results:
        for dataset, ocr_result in results["ocr_validation"].items():
            if "error" not in ocr_result:
                print(
                    f"✓ {dataset.title()} OCR: {ocr_result.get('test_cases', 0)} test cases created"
                )
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Food packaging dataset downloader and analyzer"
    )

    # Main actions
    parser.add_argument(
        "--download-off", action="store_true", help="Download Open Food Facts dataset"
    )
    parser.add_argument(
        "--download-foodiml",
        action="store_true",
    ...
    parser.add_argument(
        "--analyze", action="store_true", help="Analyze downloaded datasets"
    )
    parser.add_argument(
        "--create-ocr-tests",
        action="store_true",
    ...
    if not any(
        [
            args.download_off,
            args.download_foodiml,
            args.analyze,
            args.create_ocr_tests,
            args.run_all,
        ]
    ):
        parser.print_help()
    ...
    # Set up main logger
    logger = setup_logger(
        "main", config.log_level, log_file=config.output_dir / "dataset_pipeline.log"
    )
    ...
    try:
        if args.run_all:
            ...
            if args.download_foodiml:
                results["foodiml"] = asyncio.run(
                    download_foodiml(config, args.foodiml_sample_size)
                )
            ...
            if args.analyze:
                if args.analyze_path:
                    ...
                    for dataset_name in ["openfoodfacts", "foodiml"]:
                        ...
                        images_dir = dataset_dir / "images"
                        if images_dir.exists():
                            analysis_results[dataset_name] = asyncio.run(
                                analyze_dataset(
                                    config,
                                    images_dir,
                                    create_validation=not args.no_validation_subset,
            ...
            if args.create_ocr_tests:
                ...
                for dataset_name in ["openfoodfacts", "foodiml"]:
                    ...
                    images_dir = dataset_dir / "images"
                    if images_dir.exists():
                        ocr_results[dataset_name] = asyncio.run(
                            create_ocr_validation_suite(
