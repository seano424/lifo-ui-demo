...
class TestCompleteMobileScanningWorkflow:
    ...
    async def test_complete_store_scanning_workflow(
        ...
        normal_batches = [
            test_data_factory.create_inventory_batch(
                batch_id=f"normal-batch-{i}", days_to_expiry=10 + i, category="frozen"
            )
            for i in range(10)
        ]
        ...
        with patch(
            ...
            summary_response = await async_client.get(
                f"/api/v1/mobile-summary/{store_id}", headers=headers
            )
            ...
            assert (
                summary_data["store_health_score"] < 1.0
            )  # Should reflect urgent items
            ...
            assert (
                score_data["discount_percent"] == 0
            )  # No discounts on expired products

            # Step 4: Check store health after scanning
            health_response = await async_client.get(
                f"/api/v1/store-health/{store_id}", headers=headers
            )
    ...
    async def test_mobile_scanning_with_cache_optimization(
        ...
        with patch(
            ...
            assert cache_improvement > 2, (
                f"Cache should provide >2x improvement, got {cache_improvement:.1f}x"
            )
...
class TestMobileScanningPerformanceIntegration:
    ...
    async def test_high_volume_store_scanning(
        ...
        with patch(
            ...
            mock_opt_instance = AsyncMock()
            mock_opt_instance.get_store_inventory_fast.return_value = large_inventory[
                :200
            ]  # Mobile limit
            mock_opt_instance.get_store_health_metrics.return_value = {
                "total_batches": 1000,
    ...
    async def test_concurrent_mobile_scanning(
        ...
        with patch(
            ...
            responses = await asyncio.gather(
                *[scan_store(store_id) for store_id in stores]
            )
            ...
            assert avg_time_per_request < 400, (
                f"Concurrent requests too slow: {avg_time_per_request:.1f}ms avg"
            )
...
class TestMobileScanningDataIntegrity:
    ...
    async def test_mobile_data_consistency_across_endpoints(
        ...
        with patch(
            ...
            # Get batch list
            batch_list_response = await async_client.get(
                f"/api/v1/batch-list-mobile/{store_id}", headers=headers
            )
            assert batch_list_response.status_code == 200
            ...
            assert summary_data["total_active_batches"] == len(
                batch_list_data["batches"]
            )
            ...
            batch_list_urgent_count = len(
                [
                    b
                    for b in batch_list_data["batches"]
    ...
    async def test_mobile_filtering_accuracy(
        ...
        with patch(
            ...
            for category in categories:
                ...
                expected_count = len(
                    [b for b in test_batches if b["category"] == category]
                )
            ...
            response = await async_client.get(
                f"/api/v1/batch-list-mobile/{store_id}?limit=5", headers=headers
            )
            assert response.status_code == 200
            data = response.json()
