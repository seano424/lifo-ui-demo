...
class MobileUser(HttpUser):
    ...
    def on_start(self):
        ...
        response = self.client.post(
            "/api/v1/auth/login", json={"email": "test@lifo.ai", "password": "test123"}
        )
...
class CSVProcessingUser(HttpUser):
    ...
    def generate_csv_content(self, rows: int = 100) -> str:
        ...
        lines = [
            "sku,product_name,category,quantity,expiry_date,cost_price,selling_price"
        ]

        for i in range(rows):
            expiry = (datetime.now() + timedelta(days=random.randint(1, 30))).strftime(
                "%Y-%m-%d"
            )
            lines.append(
...
class HealthCheckUser(HttpUser):
    ...
    def basic_health(self):
        ...
        with self.client.get(
            "/api/v1/health", catch_response=True, name="/health"
        ) as response:
    ...
    def detailed_health(self):
        ...
        with self.client.get(
            "/api/v1/health/detailed", catch_response=True, name="/health/detailed"
        ) as response:
    ...
    def readiness_check(self):
        ...
        with self.client.get(
            "/api/v1/health/ready", catch_response=True, name="/health/ready"
        ) as response:
