...
class TestBoundedCachePerformance:
    ...
    async def test_cache_lru_eviction_performance(self):
        ...
        assert avg_eviction_time < 10, f"Average eviction too slow: {avg_eviction_time:.1f}ms"
        assert max_eviction_time < 50, f"Max eviction too slow: {max_eviction_time:.1f}ms"
...
class TestMobileQueryOptimization:
    ...
    async def test_mobile_cache_decorator_performance(self):
        ...
        assert performance_improvement > 5, f"Cache should provide >5x improvement, got {performance_improvement:.1f}x"
    ...
    async def test_concurrent_cache_access(self):
        ...
        for op_id, timestamps in results_by_id.items():
            assert len(set(timestamps)) == 1, f"Operation {op_id} timestamps not cached: {timestamps}"
...
class TestMobileEndpointPerformanceIntegration:
    ...
    async def test_mobile_summary_database_optimization(
        ...
        with patch('app.utils.mobile_queries.create_mobile_query_optimizer') as mock_optimizer:
            ...
            mock_opt_instance.get_store_inventory_fast.side_effect = fast_inventory_query
            ...
            for _ in range(5):
                performance_timer.start()
                response = await async_client.get("/api/v1/mobile-summary/test-store-123")
                performance_timer.stop()
            ...
            assert avg_response_time < 200, f"Average response time too slow: {avg_response_time:.1f}ms"
            assert max_response_time < 300, f"Max response time too slow: {max_response_time:.1f}ms"
            
            # Performance should be consistent (low variance)
            response_variance = sum((t - avg_response_time) ** 2 for t in response_times) / len(response_times)
            assert response_variance < 10000, f"Response time too variable: {response_variance:.1f}"
    ...
    async def test_batch_scoring_performance_optimization(
        ...
        with patch('app.utils.mobile_queries.create_mobile_query_optimizer') as mock_optimizer:
            ...
            assert avg_time_per_request < 200, f"Average scoring too slow: {avg_time_per_request:.1f}ms"
    ...
    async def test_cache_warming_performance(self):
        ...
        with patch('app.utils.performance.warm_mobile_cache') as mock_warm_cache:
            ...
            async def simulate_cache_warming(store_id, read_ops):
                ...
                # Populate test cache
                await mobile_cache.set(f"mobile_summary_{store_id}", {"cached": True}, ttl=300)
                await mobile_cache.set(f"store_health_{store_id}", {"health": 0.95}, ttl=300)
...
class TestSystemPerformanceHealth:
    ...
    async def test_mobile_performance_health_check(self):
        ...
        with patch('app.utils.performance.performance_monitor') as mock_monitor:
            ...
            required_fields = [
                "performance_summary", "cache_statistics", 
                "performance_issues", "overall_health", "memory_leak_fixed"
            ]
