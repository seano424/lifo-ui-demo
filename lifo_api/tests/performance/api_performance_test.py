...
class APIPerformanceTestSuite:
    ...
    def __init__(self, base_url: str = "http://localhost:8000"):
        ...
        self.results = {}
        self.test_user_id = "test_user_" + "".join(
    ...
    async def test_health_endpoint(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            if health_times:
                ...
                self.results["health_p95"] = sorted(health_times)[
                    int(len(health_times) * 0.95)
                ]
                self.results["health_p99"] = sorted(health_times)[
                    int(len(health_times) * 0.99)
                ]
                ...
                print(
                    "    ✓ PASS"
                    if self.results["health_avg"] < PERFORMANCE_TARGETS["health_check"]
                    else "    ✗ FAIL"
                )
        ...
        async with httpx.AsyncClient() as client:
            ...
            for i in range(20):
                ...
                try:
                    ...
                    if i < 5:
                        print(
                            f"  Iteration {i + 1}: {elapsed:.2f}ms (status: {response.status_code})"
                        )
                except Exception as e:
                    print(f"  Error in iteration {i + 1}: {e}")
            ...
            if mobile_times:
                ...
                self.results["mobile_summary_p95"] = sorted(mobile_times)[
                    int(len(mobile_times) * 0.95)
                ]
                ...
                print(
                    "    ✓ PASS"
                    if self.results["mobile_summary_avg"]
                    < PERFORMANCE_TARGETS["mobile_summary"]
                    else "    ✗ FAIL"
                )
            ...
            if quick_score_times:
                self.results["batch_quick_score_avg"] = statistics.mean(
                    quick_score_times
                )
                self.results["batch_quick_score_p95"] = sorted(quick_score_times)[
                    int(len(quick_score_times) * 0.95)
                ]
                ...
                print(
                    "    ✓ PASS"
                    if self.results["batch_quick_score_avg"]
                    < PERFORMANCE_TARGETS["batch_quick_score"]
                    else "    ✗ FAIL"
                )
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            for i in range(5):
                ...
                try:
                    ...
                    print(
                        f"  Iteration {i + 1}: {elapsed:.2f}ms for 71 batches (status: {response.status_code})"
                    )
                except Exception as e:
                    print(f"  Error in iteration {i + 1}: {e}")
            ...
            if bulk_times:
                ...
                self.results["bulk_scoring_p95"] = (
                    sorted(bulk_times)[int(len(bulk_times) * 0.95)]
                    if len(bulk_times) > 1
                    else bulk_times[0]
                )
                ...
                print(
                    "    ✓ PASS"
                    if self.results["bulk_scoring_avg"]
                    < PERFORMANCE_TARGETS["bulk_scoring"]
                    else "    ✗ FAIL"
                )
                ...
                baseline = 5000
                improvement = (
                    (baseline - self.results["bulk_scoring_avg"]) / baseline
                ) * 100
    ...
    async def test_analytics_endpoints(self):
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if analytics_times:
                self.results["analytics_dashboard_avg"] = statistics.mean(
                    analytics_times
                )
                self.results["analytics_dashboard_p95"] = (
                    sorted(analytics_times)[int(len(analytics_times) * 0.95)]
                    if len(analytics_times) > 1
                    else analytics_times[0]
                )
                ...
                print(
                    "    ✓ PASS"
                    if self.results["analytics_dashboard_avg"]
                    < PERFORMANCE_TARGETS["analytics_dashboard"]
                    else "    ✗ FAIL"
                )
    ...
    async def test_csv_processing(self):
        ...
        for i in range(100):
            csv_lines.append(
                f"Product {i},{random.randint(1, 100)},{random.uniform(10, 1000):.2f},{(datetime.now() + timedelta(days=random.randint(1, 365))).strftime('%Y-%m-%d')},Warehouse"
            )
        csv_content = "\n".join(csv_lines)
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if validation_times:
                ...
                print(
                    "    ✓ PASS"
                    if self.results["csv_validation_avg"]
                    < PERFORMANCE_TARGETS["csv_validation"]
                    else "    ✗ FAIL"
                )
            ...
            if upload_times:
                ...
                print(
                    "    ✓ PASS"
                    if self.results["csv_upload_avg"]
                    < PERFORMANCE_TARGETS["csv_upload"]
                    else "    ✗ FAIL"
                )
    ...
    async def test_multiport_servers(self):
        ...
        async with httpx.AsyncClient() as client:
            for port in ports:
                ...
                if health_times:
                    ...
                    print(
                        f"    Success Rate: {port_results[port]['success_rate']:.0f}%"
                    )
                else:
        ...
        # Count active ports
        active_ports = len(
            [p for p in port_results if port_results[p]["success_rate"] > 50]
        )
    ...
    async def test_concurrent_load(self):
        ...
        async def make_request(
            client: httpx.AsyncClient, endpoint: str, method: str = "GET", **kwargs
        ):
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            for level in concurrency_levels:
                ...
                for _ in range(level):
                    tasks.append(make_request(client, f"{self.base_url}/api/v1/health"))
    ...
    def generate_performance_report(self):
        ...
        metrics = [
            (
                "Health Check",
                self.results.get("health_avg"),
                PERFORMANCE_TARGETS.get("health_check", 50),
            ),
            (
                "Mobile Summary",
                self.results.get("mobile_summary_avg"),
                PERFORMANCE_TARGETS["mobile_summary"],
            ),
            (
                "Batch Quick Score",
                self.results.get("batch_quick_score_avg"),
                PERFORMANCE_TARGETS["batch_quick_score"],
            ),
            (
                "Bulk Scoring (71 batches)",
                self.results.get("bulk_scoring_avg"),
                PERFORMANCE_TARGETS["bulk_scoring"],
            ),
            (
                "Analytics Dashboard",
                self.results.get("analytics_dashboard_avg"),
                PERFORMANCE_TARGETS["analytics_dashboard"],
            ),
            (
                "CSV Validation",
                self.results.get("csv_validation_avg"),
                PERFORMANCE_TARGETS["csv_validation"],
            ),
            (
                "CSV Upload",
                self.results.get("csv_upload_avg"),
                PERFORMANCE_TARGETS["csv_upload"],
            ),
        ]
        ...
        for metric_name, actual, target in metrics:
            if actual is None:
                ...
                if actual < target:
                    ...
                    failed += 1
                print(
                    f"{metric_name:30} {actual:>10.2f}ms  Target: <{target}ms  {status}"
                )
        ...
        for level in [1, 5, 10, 20, 50]:
            if f"concurrent_{level}_avg" in self.results:
                ...
                print(
                    f"{level:2d} concurrent: Avg {avg:>7.2f}ms, P95 {p95:>7.2f}ms, Success {success:>3.0f}%"
                )
        ...
        if "multiport_results" in self.results:
            ...
            for port, stats in self.results["multiport_results"].items():
                if stats["success_rate"] > 50:
                    active_count += 1
                    print(
                        f"Port {port}: ✅ ACTIVE (Avg: {stats['avg']:.2f}ms, Success: {stats['success_rate']:.0f}%)"
                    )
                else:
