...
class TestAPIKeyAuthentication:
    ...
    async def test_valid_api_key_authentication(self, async_client):
        ...
        with patch('app.auth.secure_dependencies.get_current_user') as mock_auth:
            ...
            response = await async_client.get(
                "/api/v1/mobile-summary/store-123", 
                headers=headers
            )
    ...
    async def test_invalid_api_key_rejected(self, async_client):
        ...
        for invalid_key in invalid_keys:
            ...
            assert response.status_code in [401, 403], f"Failed to reject invalid key: {invalid_key}"
    ...
    async def test_api_key_format_validation(self, async_client):
        ...
        for headers in malformed_headers:
            ...
            assert response.status_code in [401, 403], f"Failed to reject malformed header: {headers}"
    ...
    async def test_authentication_error_messages_not_verbose(self, async_client):
        ...
        if response.status_code in [401, 403]:
            ...
            forbidden_info = [
                "database", "table", "query", "internal", "secret", 
                "config", "environment", "key format", "validation"
            ]
    ...
    async def test_store_access_authorization(self, async_client):
        ...
        with patch('app.auth.secure_dependencies.get_current_user') as mock_auth:
            ...
            response = await async_client.get(
                "/api/v1/mobile-summary/store-456", 
                headers=headers
            )
...
class TestAuthenticationSecurityMeasures:
    ...
    async def test_authentication_timing_attack_resistance(self, async_client):
        ...
        if len(response_times) > 1:
            ...
            assert max_deviation < 0.1, f"Response time variation too high: {max_deviation:.3f}s"
    ...
    async def test_rate_limiting_on_authentication_failures(self, async_client):
        ...
        for i in range(20):  # Try 20 failed attempts
            ...
            if response.status_code in [401, 403]:
                ...
                assert failed_attempts < 20, "Rate limiting should have kicked in before 20 attempts"
                break
...
class TestCORSSecurityConfiguration:
    ...
    async def test_cors_origin_validation(self, async_client):
        ...
        response = await async_client.options("/api/v1/mobile-summary/store-123", headers=headers)
    ...
    async def test_cors_credentials_security(self, async_client):
        ...
        if "Access-Control-Allow-Credentials" in response.headers:
            credentials_allowed = response.headers["Access-Control-Allow-Credentials"].lower()
...
class TestEndToEndAuthenticationSecurity:
    ...
    async def test_api_key_lifecycle_security(self, async_client):
        ...
        with patch('app.auth.secure_dependencies.get_current_user') as mock_auth:
            ...
            mock_auth.side_effect = HTTPException(status_code=403, detail="Token revoked")
        ...
        async def make_request(auth_state):
            if auth_state == "valid":
                with patch('app.auth.secure_dependencies.get_current_user') as mock_auth:
                    ...
                    return await async_client.get("/api/v1/mobile-summary/store-123", headers=headers)
            else:
    ...
    async def test_authentication_error_handling_security(self, async_client):
        ...
        auth_scenarios = [
            ...
            {"Authorization": "Bearer sql-injection'; DROP TABLE users;--"},  # SQL injection attempt
            {"Authorization": "Bearer <script>alert('xss')</script>"},  # XSS attempt
            {},  # No header
        ]
        
        for headers in auth_scenarios:
            response = await async_client.get("/api/v1/mobile-summary/store-123", headers=headers)
            ...
            if response.status_code != 403:  # 403 might not have body
                try:
                    ...
                    sensitive_terms = [
                        "database", "table", "column", "query", "internal",
                        "secret", "config", "environment", "stack trace", "exception"
                    ]
