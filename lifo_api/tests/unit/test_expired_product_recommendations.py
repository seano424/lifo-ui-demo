...
class TestExpiredProductRecommendations:
    ...
    def test_expired_product_eu_compliance_requirements(self):
        ...
        recommendation = scorer._generate_expired_recommendation(
            days_to_expiry=0,
            current_margin_percent=75.0,
            current_quantity=50.0
        )
        
        # Verify EU compliance requirements
        assert recommendation["action"] == "dispose", "EU law requires disposal of expired products"
        assert recommendation["urgency"] == "critical", "Expired products must be highest urgency"
        assert recommendation["reason"] == "product expired", "Must clearly state expiration reason"
        assert recommendation["discount_percent"] == 0, "No discounts allowed on expired products"
        assert recommendation["priority"] == 1, "Expired products must be highest priority"
    ...
    def test_expired_product_recommendation_immutability(self):
        ...
        for _ in range(5):
            ...
            assert recommendation == baseline, "Recommendations should be consistent across instances"
    ...
    def test_expired_product_integration_with_generate_recommendation(self):
        ...
        assert expired_recommendation["discount_percent"] == direct_recommendation["discount_percent"]
    ...
    def test_expired_vs_non_expired_distinction(self):
        ...
        assert non_expired_rec.get("discount_percent", 0) > 0  # Should have discount option
    ...
    def test_expired_product_parameter_validation(self):
        ...
        for days, margin, quantity in test_parameters:
            try:
                ...
                pytest.fail(f"Expired recommendation failed with parameters {days}, {margin}, {quantity}: {e}")
...
class TestExpiredProductCompliance:
    ...
    def test_expired_product_traceability_requirements(self):
        ...
        assert "reason" in recommendation, "Must include reason for regulatory compliance"
        ...
        assert "priority" in recommendation, "Must include priority for disposal workflow"
        assert recommendation["priority"] == 1, "Expired products must be highest priority"
