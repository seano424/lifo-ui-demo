...
class TestMobileEndpointPerformance:
    ...
    async def test_mobile_summary_performance(
        self, async_client: AsyncClient, mock_api_key_auth, mobile_test_data, performance_timer, perf_assert
    ):
        """Test mobile summary endpoint meets 300ms performance target"""
        store_id = mobile_test_data["store_id"]
        expected_threshold = mobile_test_data["expected_performance_ms"]["mobile_summary"]
    ...
    async def test_batch_list_mobile_performance(
        ...
        with patch('app.utils.mobile_queries.create_mobile_query_optimizer') as mock_optimizer:
            ...
            performance_timer.start()
            response = await async_client.get(f"/api/v1/batch-list-mobile/{store_id}?limit=20")
            performance_timer.stop()
...
class TestMobileEndpointFunctionality:
    ...
    async def test_mobile_summary_response_structure(
        ...
        with patch('app.utils.mobile_queries.create_mobile_query_optimizer') as mock_optimizer:
            ...
            required_fields = [
                "urgent_batches", "expiring_today", "action_needed", 
                "total_active_batches", "store_health_score", 
                "last_updated", "cache_expires_in"
            ]
    ...
    async def test_mobile_summary_data_compression(
        ...
        with patch('app.utils.mobile_queries.create_mobile_query_optimizer') as mock_optimizer:
            ...
            assert len(str(data["store_health_score"]).split('.')[-1]) <= 2  # Max 2 decimal places
    ...
    async def test_mobile_performance_health_endpoint(
        ...
        with patch('app.utils.performance.mobile_performance_health_check') as mock_health:
            ...
            required_fields = [
                "mobile_performance_status", "query_performance", 
...
class TestMobileEndpointErrorHandling:
    ...
    async def test_mobile_endpoints_authentication_required(
        ...
        # Test POST endpoint
        response = await async_client.post("/api/v1/batch-quick-score/test-batch?store_id=test-store")
    ...
    async def test_mobile_summary_limit_parameter(
        ...
        with patch('app.utils.mobile_queries.create_mobile_query_optimizer') as mock_optimizer:
            ...
            response = await async_client.get(f"/api/v1/mobile-summary/{store_id}?limit_urgent=5")
...
class TestMobileEndpointCaching:
    ...
    async def test_quick_score_caching(
        ...
        with patch('app.utils.mobile_queries.create_mobile_query_optimizer') as mock_optimizer:
            ...
            performance_timer = performance_timer if 'performance_timer' in locals() else type('Timer', (), {
                'start': lambda: setattr(performance_timer, 'start_time', time.time()),
                'stop': lambda: setattr(performance_timer, 'end_time', time.time()),
                'elapsed_ms': property(lambda self: (self.end_time - self.start_time) * 1000 if hasattr(self, 'start_time') and hasattr(self, 'end_time') else 0)
            })()
            ...
            assert performance_timer.elapsed_ms < 50  # Should be much faster when cached