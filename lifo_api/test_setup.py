...
async def test_comprehensive():
    ...
    try:
        from app.database.supabase_service import (
        ...
        print(
            f"   Connection test: {'PASS' if health.get('connection_test') else 'FAIL'}"
        )
    ...
    try:
        ...
        try:
            result = (
                ...
                .from_("ocr_processing_batches")
                .select("*")
                .limit(1)
                .execute()
            )
            print(
                f"   ✅ inventory.ocr_processing_batches accessible ({len(result.data)} rows)"
            )
        except Exception as e:
    ...
    try:
        ...
        print(
            f"   Service role key configured: {bool(settings.supabase_service_role_key)}"
        )
