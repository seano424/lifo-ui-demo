...
from app.services.product_name_extraction_service import get_product_name_extraction_service
...
class ComprehensiveOCRValidator:
    ...
    def load_test_dataset(self):
        ...
        print(f"✅ Loaded test dataset: {self.test_dataset['test_dataset_info']['name']}")
        print(f"📊 Total test cases: {self.test_dataset['test_dataset_info']['total_test_cases']}")
        print(f"🌍 Languages: {', '.join(self.test_dataset['test_dataset_info']['languages'])}")
        print(f"🏢 Categories: {', '.join(self.test_dataset['test_dataset_info']['categories'])}")
    ...
    async def run_comprehensive_validation(self):
        ...
        for test_case in self.test_dataset['test_cases']:
            ...
            print(f"   Region: {test_case['region']}, Language: {test_case['language']}")
        ...
        try:
            ...
            for result in results:
                extracted_dates.append({
                    ...
                    'date': result.date.strftime('%Y-%m-%d') if result.date else result.raw_text,
            ...
            accuracy = min(found_count / max(expected_count, 1), 1.0) if expected_count > 0 else 1.0
            passed = accuracy >= 0.8 and performance_ms < 200  # Relaxed targets for real-world data
    ...
    async def _test_product_extraction(
        ...
        try:
            ...
            accuracy = min(found_count / max(expected_count, 1), 1.0) if expected_count > 0 else 1.0
            passed = accuracy >= 0.7 and performance_ms < 250  # More lenient for product extraction
    ...
    def _generate_comprehensive_summary(self):
        ...
        for service in services:
            performance_key = f"{service.rstrip('s')}_extraction_ms" if service != 'product_names' else 'product_extraction_ms'
            if service == 'barcodes':
        ...
        for lang, stats in language_stats.items():
            self.results.summary['language_performance'][lang] = stats['passed'] / stats['total']

        for region, stats in region_stats.items():
            self.results.summary['region_performance'][region] = stats['passed'] / stats['total']
    ...
    def _print_comprehensive_summary(self):
        ...
        for service, avg_time in summary['performance_metrics'].items():
            target = {'dates': 100, 'barcodes': 150, 'product_names': 200}.get(service, 200)
            ...
            for test in failed_tests:
                ...
                for service in ['dates', 'barcodes', 'product_names']:
                    ...
                    if not result.get('passed', True):
                        ...
                        print(f"     {service}: {accuracy:.1%} accuracy, {performance:.1f}ms")
        ...
        slow_services = [
            service for service, time in summary['performance_metrics'].items()
            if time > {'dates': 150, 'barcodes': 200, 'product_names': 300}.get(service, 200)
        ]
        if slow_services:
            insights.append(f"⚡ Performance optimization needed for: {', '.join(slow_services)}")
    ...
    async def export_detailed_results(self, filename: str = "ocr_validation_results.json"):
