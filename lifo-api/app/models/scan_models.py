...
class ScanInRequest(BaseModel):
    ...
    selling_price: Optional[float] = Field(None, ge=0, le=10000, description="Selling price per unit")
...
class ScanOutRequest(BaseModel):
    ...
    destination_location: Optional[str] = Field(None, max_length=50, description="Destination location")
...
class ProcessScanRequest(BaseModel):
    ...
    scan_timestamp: Optional[datetime] = Field(default_factory=datetime.utcnow, description="When scan occurred")
...
class ScanWorkflowException(MVPException):
    ...
    def __init__(self, message: str, workflow: str, error_code: str = "SCAN_ERROR", status_code: int = 400):
