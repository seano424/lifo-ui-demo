...
from app.auth.supabase_jwt import supabase_auth, SupabaseUser, SupabaseAuthError
...
async def validate_store_access(
    ...
    try:
        ...
        if not store_user:
            logger.warning("User access denied - no store association",
        ...
        if user_role_level < required_role_level:
            logger.warning("User access denied - insufficient role",
                         user_id=current_user.user_id,
                         store_id=store_id,
        ...
        logger.info("Store access validated",
                   user_id=current_user.user_id,
                   store_id=store_id,
                   user_role=store_user.role_in_store)
        ...
        logger.error("Store access validation failed",
                    user_id=current_user.user_id,
                    store_id=store_id,
                    error=str(e))
        raise HTTPException(
...
async def get_user_stores(
    ...
    try:
        ...
        logger.info("Retrieved user stores",
                   user_id=current_user.user_id,
        ...
        logger.error("Failed to get user stores",
                    user_id=current_user.user_id,
                    error=str(e))
        return []
