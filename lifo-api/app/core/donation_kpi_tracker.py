...
class DonationKPITracker:
    ...
    def calculate_comprehensive_kpis(self,
                                   ...
                                   period_start: date,
                                   period_end: date,
                                   store_id: str) -> ComprehensiveKPIReport:
        """
        ...
        try:
            ...
            for donation in donations_data:
                category_breakdown[donation.category] = category_breakdown.get(donation.category, 0) + 1
            
            # Generate trends and insights
            trends = self._analyze_trends(donations_data, timeframe)
            insights = self._generate_insights(
                financial_metrics, environmental_metrics, social_metrics, 
                compliance_metrics, efficiency_metrics, total_donations
            )
            ...
            self.logger.info("Comprehensive KPI report calculated",
                           store_id=store_id,
            ...
            self.logger.error("KPI calculation failed",
                            store_id=store_id,
                            error=str(e))
            raise
    
    def _calculate_financial_metrics(self, donations_data: List[DonationImpactData]) -> FinancialImpactMetrics:
        ...
        return FinancialImpactMetrics(
            ...
            cost_per_kg_donated=Decimal(str(sum(d.cost_price * d.quantity_donated for d in donations_data) / total_quantity))
        )
    
    def _calculate_environmental_metrics(self, donations_data: List[DonationImpactData]) -> EnvironmentalImpactMetrics:
        ...
        for donation in donations_data:
            factors = self.environmental_factors.get(donation.category, self.environmental_factors['fresh_produce'])
        ...
        # Carbon credit value
        carbon_credit_value = total_co2_avoided * self.financial_factors['carbon_credit_eur_per_kg_co2']
    ...
    def _calculate_social_metrics(self, donations_data: List[DonationImpactData]) -> SocialImpactMetrics:
        ...
        for donation in donations_data:
            ...
            nutritional_multiplier = self.social_factors['nutritional_value_multiplier'].get(donation.category, 1.0)
            
            meals = donation.quantity_donated * meals_per_kg * nutritional_multiplier
            total_meals += int(meals)
            
            social_value = donation.original_value * self.financial_factors['social_value_multiplier']
    ...
    def _calculate_eu_compliance_metrics(self, donations_data: List[DonationImpactData]) -> EUComplianceMetrics:
        ...
        animal_product_donations = [d for d in donations_data if d.category in ['fresh_meat_fish', 'dairy']]
        regulation_853_compliance = 1.0 if not animal_product_donations else (
            sum(1 for d in animal_product_donations if d.eu_compliance_score >= 0.95) / len(animal_product_donations)
        )
    ...
    def _calculate_operational_efficiency_metrics(self, donations_data: List[DonationImpactData]) -> OperationalEfficiencyMetrics:
        ...
        return OperationalEfficiencyMetrics(
            avg_time_identification_to_donation_hours=sum(identification_to_donation_times) / len(identification_to_donation_times) if identification_to_donation_times else 0.0,
            avg_time_approval_to_pickup_hours=sum(approval_to_pickup_times) / len(approval_to_pickup_times) if approval_to_pickup_times else 0.0,
            avg_time_pickup_to_delivery_hours=sum(pickup_to_delivery_times) / len(pickup_to_delivery_times) if pickup_to_delivery_times else 0.0,
    ...
    def _analyze_trends(self, donations_data: List[DonationImpactData], timeframe: KPITimeframe) -> Dict[str, str]:
        ...
        value_trend = "increasing" if second_half_avg_value > first_half_avg_value * 1.1 else \
                     "decreasing" if second_half_avg_value < first_half_avg_value * 0.9 else "stable"
        
        compliance_trend = "improving" if second_half_compliance > first_half_compliance + 0.05 else \
                          "declining" if second_half_compliance < first_half_compliance - 0.05 else "stable"
        
        return {
            "donation_value": value_trend,
            "compliance_score": compliance_trend,
            "overall": "positive" if value_trend == "increasing" and compliance_trend in ["improving", "stable"] else "stable"
        }
    
    def _generate_insights(self, financial: FinancialImpactMetrics, environmental: EnvironmentalImpactMetrics,
                          social: SocialImpactMetrics, compliance: EUComplianceMetrics,
                          efficiency: OperationalEfficiencyMetrics, total_donations: int) -> List[str]:
        """Generate key insights from metrics"""
        insights = []
        
        # Financial insights
        if financial.net_financial_benefit > 0:
            insights.append(f"Donation program generates positive financial impact of €{financial.net_financial_benefit:.2f}")
        
        # Environmental insights
        if environmental.co2_emissions_avoided_kg > 100:
            insights.append(f"Significant environmental impact: {environmental.co2_emissions_avoided_kg:.1f}kg CO2 emissions avoided")
        
        # Social insights
        if social.estimated_meals_provided > 50:
            insights.append(f"Strong social impact: {social.estimated_meals_provided} meals provided to community")
        
        # Compliance insights
        if compliance.overall_compliance_rate < 0.95:
            insights.append(f"Compliance rate of {compliance.overall_compliance_rate:.1%} needs improvement")
    ...
    def _generate_recommendations(self, compliance: EUComplianceMetrics, 
                                efficiency: OperationalEfficiencyMetrics) -> List[str]:
        ...
        if compliance.temperature_compliance_rate < 0.9:
            recommendations.append("Upgrade temperature monitoring equipment for better cold chain compliance")
        ...
        if compliance.critical_violations > 0:
            recommendations.append("Implement immediate corrective action plan for critical compliance violations")
    ...
    def _assess_data_quality(self, donations_data: List[DonationImpactData]) -> float:
        ...
        # Completeness check
        complete_records = sum(1 for d in donations_data if all([
            d.donation_id, d.batch_id, d.quantity_donated > 0,
            d.original_value > 0, d.donation_timestamp
        ]))
