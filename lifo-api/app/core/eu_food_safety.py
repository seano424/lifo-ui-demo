...
class EUFoodSafetyValidator:
    ...
    def __init__(self):
        ...
        self.eu_safety_thresholds = {
            EUFoodCategory.FRESH_MEAT_FISH: {
                ...
                'recipient_restrictions': ['food_bank_certified', 'soup_kitchen_licensed']
            },
    ...
    def validate_donation_eligibility(self, 
                                    category: str,
                                    expiry_date: date,
                                    current_temperature: Optional[float] = None,
                                    packaging_condition: str = "good") -> EUComplianceResult:
        """
        ...
        try:
            ...
            if days_to_expiry < min_days:
                ...
                regulatory_notes.append(f"Product must have at least {min_days} days before expiry for EU-compliant donation")
            ...
            if days_to_expiry < 0:
                ...
                eligibility_status = DonationEligibility.NOT_ELIGIBLE_EXPIRED
                compliance_score = 0.0
                regulatory_notes.append("Expired products cannot be donated under EU food safety regulations")
            
            # Check packaging condition
            if packaging_condition in ["damaged", "opened"]:
                if eu_category in [EUFoodCategory.FRESH_MEAT_FISH, EUFoodCategory.DAIRY, EUFoodCategory.DELI_PREPARED]:
                    eligible = False
                    eligibility_status = DonationEligibility.NOT_ELIGIBLE_SAFETY
                    compliance_score = 0.0
                    regulatory_notes.append("Damaged or opened packaging not permitted for animal products under EU Regulation 853/2004")
                else:
                    eligible = True
                    ...
                    handling_instructions.append("Inspect product condition thoroughly before donation")
            
            # Temperature compliance checks
            if safety_config['temperature_critical'] and current_temperature is not None:
                temp_compliance = self._check_temperature_compliance(eu_category, current_temperature)
                if not temp_compliance['compliant']:
                    eligible = False
                    eligibility_status = DonationEligibility.NOT_ELIGIBLE_SAFETY
                    compliance_score = 0.0
                    regulatory_notes.append(f"Temperature violation: {temp_compliance['violation']}")
                else:
            ...
            temp_requirements = self._get_temperature_requirements(eu_category) if safety_config['temperature_critical'] else None
            ...
            if eligible:
                ...
                if eu_category in [EUFoodCategory.FRESH_MEAT_FISH, EUFoodCategory.DAIRY, EUFoodCategory.DELI_PREPARED]:
                    regulatory_notes.append("Compliant with EU Regulation 853/2004 (Animal Products)")
            
            self.logger.info("EU donation eligibility assessed",
                           category=category,
            ...
            self.logger.error("EU donation eligibility validation failed",
                            category=category,
                            error=str(e))
            return EUComplianceResult(
                ...
                safety_requirements=[],
                regulatory_notes=[f"Validation error: {str(e)}"],
    ...
    def _check_temperature_compliance(self, eu_category: EUFoodCategory, temperature: float) -> Dict[str, Any]:
        ...
        requirements = [f"Maintain temperature between {range_config['min']}-{range_config['max']}°C"]
    ...
    def get_donation_priority_score(self, 
                                  category: str,
                                  days_to_expiry: int,
                                  compliance_result: EUComplianceResult) -> float:
        """
