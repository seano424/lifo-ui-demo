...
class SecureCSVProcessor:
    ...
    ALLOWED_COLUMNS = {
        'sku', 'product_name', 'category', 'brand', 'quantity', 
        'cost_price', 'selling_price', 'expiry_date', 'batch_number', 
        'location_code', 'supplier', 'description', 'unit_type'
    }
    ...
    def _decode_csv_content_secure(self, content: bytes) -> str:
        ...
        if len(content) > self.MAX_FILE_SIZE:
            raise HTTPException(
                status_code=413,
                detail=f"File content too large: {len(content)} bytes"
            )
    ...
    def _parse_csv_structure_secure(self, csv_content: str) -> Dict[str, Any]:
        ...
        if self._has_suspicious_content(csv_content):
            raise HTTPException(
                status_code=400,
                detail="CSV contains suspicious content"
            )
    ...
    def _has_suspicious_content(self, content: str) -> bool:
        ...
        special_char_count = sum(1 for char in content if not char.isalnum() and char not in ' ,.:-_\n\r\t"')
    ...
    async def _validate_and_sanitize_csv(self, parsed_data: Dict[str, Any], store_id: str) -> Dict[str, Any]:
        ...
        required_columns = ['sku', 'product_name', 'quantity', 'cost_price', 'selling_price']
        missing_required = []
    ...
    async def _validate_row_secure(self, row_data: Dict[str, str], row_number: int) -> Dict[str, Any]:
        ...
        if 'sku' in row_data:
            ...
            if not re.match(r'^[A-Z0-9\-_]+$', sku):
                raise ValueError(f"Invalid SKU: only letters, numbers, hyphens, and underscores allowed")
        ...
        if 'expiry_date' in row_data and row_data['expiry_date']:
            try:
                ...
                raise ValueError(f"Invalid expiry date format: {row_data['expiry_date']}. Use YYYY-MM-DD")
        ...
        if 'category' in row_data:
            ...
            valid_categories = [
                ...
                'frozen', 'canned_jarred', 'dry_goods', 'beverages',
                'deli_prepared', 'spices_condiments', 'general'
            ]
    ...
    async def _validate_business_rules_secure(self, row_data: Dict[str, Any], store_id: str, row_number: int) -> Dict[str, List]:
    ...
    def generate_secure_csv_template(self) -> str:
        ...
        headers = ["sku", "product_name", "quantity", "cost_price", "selling_price", "expiry_date", "category", "brand", "location_code"]
        
        sample_data = [
            ["DAIRY-001", "Organic Milk 1L", "24", "1.20", "2.50", "2024-12-25", "dairy", "Farm Fresh", "FRIDGE-A1"],
            ["BREAD-001", "Whole Wheat Bread", "15", "0.80", "1.80", "2024-12-20", "bakery_fresh", "Local Bakery", "SHELF-B2"]
        ]
