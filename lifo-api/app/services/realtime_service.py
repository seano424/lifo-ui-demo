...
class RealtimeService:
    ...
    async def subscribe_to_store(self, store_id: str, user_id: str):
        ...
        if user_id not in self.active_subscriptions[store_id]:
            ...
            self.logger.info("User subscribed to store updates", 
    ...
    async def unsubscribe_from_store(self, store_id: str, user_id: str):
        ...
        if store_id in self.active_subscriptions:
            if user_id in self.active_subscriptions[store_id]:
                ...
                self.logger.info("User unsubscribed from store updates", 
                               store_id=store_id, user_id=user_id)
    ...
    async def broadcast_update(self, update: RealtimeUpdate):
        ...
        # Log the broadcast
        self.logger.info("Update broadcast", 
                        store_id=store_id, 
    ...
    async def _send_to_supabase_realtime(self, update: RealtimeUpdate, subscribers: List[str]):
        ...
        try:
            ...
            self.logger.debug("Real-time payload prepared", 
                            store_id=update.store_id,
                            payload_size=len(json.dumps(payload)))
            
        except Exception as e:
            self.logger.error("Failed to send real-time update", 
                            store_id=update.store_id,
                            error=str(e))
    
    async def trigger_score_update(self, store_id: str, batch_id: str, 
                                 score_data: Dict[str, Any], priority: str = "normal"):
    ...
    async def trigger_scan_completion(self, store_id: str, batch_id: str, 
                                    scan_data: Dict[str, Any], scan_type: str):
    ...
    async def trigger_urgency_alert(self, store_id: str, batch_id: str, 
    ...
    async def trigger_batch_status_change(self, store_id: str, batch_id: str, 
    ...
    async def trigger_new_batch_alert(self, store_id: str, batch_id: str, 
    ...
    async def process_update_queue(self, max_batch_size: int = 50):
        ...
        self.logger.info("Update queue processed", 
    ...
    async def _process_store_updates(self, store_id: str, updates: List[RealtimeUpdate]):
        ...
        try:
            ...
            self.logger.error("Failed to process store updates", 
                            store_id=store_id, 
                            error=str(e))
...
async def notify_scan_completion(store_id: str, batch_id: str, scan_result: Dict[str, Any], scan_type: str):
    ...
    try:
        if workflow_type == "scan_in":
            ...
            if workflow_data.get("initial_score", 0) >= 0.8:
                await notify_urgency_alert(store_id, batch_id, {
            ...
            await notify_batch_change(store_id, batch_id, {
        ...
        logger.error("Real-time integration failed", 
