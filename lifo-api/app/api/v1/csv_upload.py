...
try:
    from etl.unified_csv_processor import UnifiedCSVProcessor, ProcessingResult
...
class FastAPICSVIntegration:
    ...
    async def process_csv_upload(
        ...
        if not UnifiedCSVProcessor:
            raise HTTPException(
                status_code=500,
                detail="Unified CSV processor not available"
            )
        ...
        "data": {
            ...
            "instructions": {
                ...
                "category_examples": [
                    ...
                    "frozen", "beverages", "dry_goods", "canned_jarred"
                ],
    ...
    try:
        ...
        return {
            ...
            "validation_results": {
                ...
                "errors": result.get("errors", []),
                "warnings": result.get("warnings", []),
                "preview_data": result["data"][:5] if result["data"] else [],  # First 5 rows for preview
                "metadata": result.get("metadata", {})
            },
            "message": "Validation completed successfully" if result["status"] != "error" else "Validation found errors"
        }
