...
from app.middleware.rate_limiting import csv_processing_rate_limit, ai_endpoint_rate_limit
...
async def validate_csv_secure(
    ...
    try:
        ...
        logger.info("CSV validation completed securely", 
                   store_id=store_id, 
        ...
        logger.error("Secure CSV validation failed", 
                    store_id=store_id, 
...
async def get_secure_csv_template(
    ...
    try:
        ...
        logger.info("Secure CSV template generated", 
                   user_id=current_user["sub"])
        
        return template_data
        
    except Exception as e:
        logger.error("Failed to generate secure CSV template", 
                    error=str(e),
                    user_id=current_user["sub"])
...
async def analyze_csv_with_ai(
    ...
    try:
        ...
        ai_analysis = {
            ...
            "urgency_alerts": analysis_result["ai_suggestions"]["insights"].get("urgency_alerts", []),
            "category_distribution": analysis_result["ai_suggestions"]["insights"].get("categories", {}),
            "pricing_insights": analysis_result["ai_suggestions"]["insights"].get("pricing_insights", {}),
            "recommendations": [
                {
                    "type": "data_quality",
        ...
        logger.info("CSV AI analysis completed", 
                   store_id=store_id, 
                   filename=file.filename,
        ...
        logger.error("CSV AI analysis failed", 
                    store_id=store_id, 
                    filename=file.filename if file else "unknown",
                    error=str(e),
                    user_id=current_user["sub"])
