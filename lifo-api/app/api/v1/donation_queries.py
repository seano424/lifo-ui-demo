...
from app.database.donation_models import (
    DonationRecipient, DonationRecord, DonationComplianceCheck, 
    DonationKPIImpact, DonationAlert, DonationAnalytics
)
    ...
    try:
        ...
        for recipient in recipients:
            recipient_dict = {
                ...
                "last_donation_date": recipient.last_donation_date.isoformat() if recipient.last_donation_date else None
            }
...
async def get_donation_records(
    ...
    try:
        ...
        query = select(DonationRecord, DonationRecipient).join(
            DonationRecipient, DonationRecord.recipient_id == DonationRecipient.recipient_id
        )
        ...
        for donation_record, recipient in records:
            record_dict = {
                ...
                "donation_date": donation_record.donation_date.isoformat() if donation_record.donation_date else None,
                "total_items": donation_record.total_items,
                "total_weight_kg": float(donation_record.total_weight_kg) if donation_record.total_weight_kg else None,
                "estimated_value_eur": float(donation_record.estimated_value_eur) if donation_record.estimated_value_eur else None,
                "status": donation_record.status,
                "pickup_scheduled": donation_record.pickup_scheduled.isoformat() if donation_record.pickup_scheduled else None,
                "pickup_completed": donation_record.pickup_completed.isoformat() if donation_record.pickup_completed else None,
                "compliance_status": donation_record.compliance_status,
                "created_at": donation_record.created_at.isoformat() if donation_record.created_at else None
            }
...
async def get_compliance_checks(
    ...
    try:
        query = select(DonationComplianceCheck).where(
            DonationComplianceCheck.donation_id == donation_id
        ...
        for check in checks:
            check_dict = {
                ...
                "check_performed_at": check.check_performed_at.isoformat() if check.check_performed_at else None,
        ...
        logger.error("Error fetching compliance checks", donation_id=donation_id, error=str(e))
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_donation_analytics_summary(
    ...
    try:
        ...
        total_items_donated = sum(a.total_items_donated for a in analytics if a.total_items_donated)
        total_weight_donated = sum(float(a.total_weight_donated_kg or 0) for a in analytics)
        total_value_donated = sum(float(a.total_value_donated_eur or 0) for a in analytics)
        avg_compliance_score = sum(float(a.compliance_score or 0) for a in analytics) / len(analytics) if analytics else 0
        
        # Recent activity
        recent_query = select(DonationRecord).order_by(desc(DonationRecord.donation_date)).limit(5)
        if store_id:
...
async def get_donation_kpi_impact(
    ...
    try:
        ...
        return {
            ...
            "food_waste_reduction_kg": float(kpi_impact.food_waste_reduction_kg) if kpi_impact.food_waste_reduction_kg else None,
            "co2_emissions_saved_kg": float(kpi_impact.co2_emissions_saved_kg) if kpi_impact.co2_emissions_saved_kg else None,
            "social_impact_score": float(kpi_impact.social_impact_score) if kpi_impact.social_impact_score else None,
            "tax_benefit_eur": float(kpi_impact.tax_benefit_eur) if kpi_impact.tax_benefit_eur else None,
            "calculated_at": kpi_impact.calculated_at.isoformat() if kpi_impact.calculated_at else None
        }
