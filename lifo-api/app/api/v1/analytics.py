...
from app.auth.secure_dependencies import get_current_user, validate_store_id_format, validate_store_access
...
async def get_store_analytics(
    ...
    try:
        ...
        logger.info("Store analytics retrieved", 
                   store_id=store_id, 
                   period_days=days,
        ...
        logger.error("Failed to get store analytics", 
                    store_id=store_id, 
                    error=str(e),
                    user_id=current_user["sub"])
...
async def get_dashboard_data(
    ...
    try:
        ...
        dashboard_data = {
            ...
            "alerts": {
                "expired_items": analytics_data.get("inventory_summary", {}).get("expired_count", 0),
                "expiring_soon": analytics_data.get("inventory_summary", {}).get("expiring_soon_count", 0),
...
async def get_performance_metrics(
    ...
    try:
        ...
        successful_actions = sum(1 for action in analytics_data.get("recent_actions", []) 
        ...
        revenue_recovered = sum(action.get("new_price", 0) - action.get("original_price", 0) 
...
async def get_export_data(
    store_id: str,
    export_type: str = Query("inventory", description="Export type (inventory, analytics, actions)"),
