...
async def analyze_product_image(
    ...
    analysis_type: str = Form("expiry_date", description="Type of analysis: expiry_date, barcode, or full"),
    confidence_threshold: float = Form(0.7, ge=0.1, le=1.0, description="Minimum confidence for results"),
...
async def extract_expiry_date_from_image(
    ...
    try:
        ...
        # Mock OCR results
        ocr_results = await _mock_expiry_date_extraction(
            image_data, 
            date_format_hint
        )
        
        logger.info("Expiry date extraction completed",
                   store_id=store_id,
        ...
        logger.error("Expiry date extraction failed",
                    store_id=store_id,
                    error=str(e),
                    user_id=current_user["sub"])
        raise HTTPException(
            status_code=500,
            detail="Expiry date extraction failed"
        )
...
async def detect_barcode_from_image(
    ...
    request: Request,
    image: UploadFile = File(...),
    barcode_types: List[str] = Form(["EAN13", "UPC", "CODE128"], description="Expected barcode types"),
    db: AsyncSession = Depends(get_db),
    current_user: Dict[str, Any] = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        barcode_results = await _mock_barcode_detection(
            image_data, 
            barcode_types
        )
        
        logger.info("Barcode detection completed",
                   store_id=store_id,
        ...
        raise HTTPException(
            status_code=500,
            detail="Barcode detection failed"
        )
    ...
    try:
        ...
        model_status = {
            ...
            "product_classifier": {
                ...
                "supported_categories": ["fresh_produce", "dairy", "bakery", "packaged"]
            }
        }
        
        logger.info("ML models status requested",
                   user_id=current_user["sub"])
        ...
        logger.error("ML models status check failed",
                    error=str(e),
                    user_id=current_user["sub"])
        raise HTTPException(
            status_code=500,
