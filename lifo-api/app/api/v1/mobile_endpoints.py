...
from app.models.scan_models import (
    MobileBatchSummary, QuickBatchScore, MobileBatchItem, 
    MobileStoreHealth, MobileOptimizedError
)
...
async def get_mobile_batch_summary(
    ...
    try:
        ...
        urgent_batches = sorted(urgent_batches, key=lambda x: x["urgency_score"], reverse=True)[:limit_urgent]
        expiring_today = sorted(expiring_today, key=lambda x: x["days_to_expiry"])[:limit_urgent]
        action_needed = sorted(action_needed, key=lambda x: x["urgency_score"], reverse=True)[:limit_urgent]
        ...
        logger.info("Mobile summary generated",
                   store_id=store_id,
        ...
        processing_time_ms = (time.time() - start_time) * 1000
        logger.error("Mobile summary failed",
                    store_id=store_id,
                    error=str(e),
                    processing_time_ms=processing_time_ms,
                    user_id=current_user["sub"])
        raise HTTPException(
            status_code=500,
            detail="Failed to generate mobile summary"
        )
...
async def quick_batch_score(
    ...
    try:
        ...
        expiry_score = _calculate_quick_expiry_score(days_to_expiry, batch_data.get("typical_shelf_life_days", 30))
        velocity_score = 0.5  # Default for quick calculation
        margin_score = _calculate_quick_margin_score(batch_data["cost_price"], batch_data["selling_price"])
        ...
        urgency_level, recommendation, suggested_action, discount_suggestion = _get_quick_recommendation(
            composite_score, days_to_expiry
        )
        
        processing_time_ms = (time.time() - start_time) * 1000
        
        logger.info("Quick score calculated",
                   batch_id=batch_id,
        ...
        processing_time_ms = (time.time() - start_time) * 1000
        logger.error("Quick scoring failed",
                    batch_id=batch_id,
                    error=str(e),
                    processing_time_ms=processing_time_ms,
                    user_id=current_user["sub"])
...
async def get_mobile_store_health(
    ...
    try:
        ...
        logger.error("Store health calculation failed",
                    store_id=store_id,
                    error=str(e),
                    user_id=current_user["sub"])
        raise HTTPException(
            status_code=500,
            detail="Store health calculation failed"
        )
...
async def get_mobile_batch_list(
    ...
    try:
        ...
        logger.info("Mobile batch list generated",
                   store_id=store_id,
        ...
        processing_time_ms = (time.time() - start_time) * 1000
        logger.error("Mobile batch list failed",
                    store_id=store_id,
                    error=str(e),
                    processing_time_ms=processing_time_ms,
                    user_id=current_user["sub"])
        raise HTTPException(
            status_code=500,
            detail="Failed to get batch list"
        )
