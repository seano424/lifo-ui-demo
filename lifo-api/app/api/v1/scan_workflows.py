...
async def scan_donation_eligibility_check(
    ...
    try:
        ...
        mobile_response = {
            ...
            "requires_temperature_monitoring": recommendation.compliance_result.temperature_requirements is not None,
            "handling_priority": _get_handling_priority(recommendation.priority.value)
        }
        
        processing_time_ms = (time.time() - start_time) * 1000
        
        logger.info("Donation eligibility scan completed",
                   store_id=store_id,
                   batch_id=batch_id,
        ...
        processing_time_ms = (time.time() - start_time) * 1000
        logger.error("Donation eligibility scan failed",
                    store_id=store_id,
                    batch_id=batch_id,
                    error=str(e),
                    processing_time_ms=processing_time_ms,
        ...
        raise HTTPException(
            status_code=500,
            detail="Donation eligibility scan failed"
        )
    ...
    try:
        ...
        response = {
            ...
            "compliance_summary": {
                ...
                "documentation_required": len(recommendation.compliance_result.safety_requirements) > 0
            },
            ...
            "mobile_feedback": {
                ...
                "badge_earned": "EU Compliant Donor" if recommendation.compliance_result.compliance_score >= 0.9 else None
            },
        ...
        logger.info("Donation action executed via mobile scan",
                   store_id=store_id,
                   batch_id=batch_id,
                   donation_id=donation_record["donation_id"],
        ...
        processing_time_ms = (time.time() - start_time) * 1000
        logger.error("Donation action execution failed",
                    store_id=store_id,
                    batch_id=batch_id,
                    error=str(e),
                    processing_time_ms=processing_time_ms,
        ...
        raise HTTPException(
            status_code=500,
            detail="Donation action execution failed"
        )
...
async def get_donation_quick_scan_list(
    store_id: str,
    request: Request,
    priority_filter: Optional[str] = Query("high", description="Priority filter: critical, high, medium, all"),
    ...
    try:
        ...
        for batch in inventory_data:
            try:
                ...
                if recommendation.eu_compliant:
                    ...
                    if priority_filter == "critical" and priority_value != "critical":
                        continue
                    elif priority_filter == "high" and priority_value not in ["critical", "high"]:
                        continue
                    elif priority_filter != "all" and priority_filter != priority_value:
                        continue
                    ...
                    candidate = {
                        ...
                        "temperature_sensitive": recommendation.compliance_result.temperature_requirements is not None,
                        "quick_action": priority_value in ["critical", "high"] and batch.get("days_to_expiry", 999) <= 2
                    }
                    
                    donation_candidates.append(candidate)
                    
            except Exception as e:
                logger.warning("Failed to evaluate batch for donation",
                             batch_id=batch.get("batch_id"),
                             error=str(e))
                continue
        ...
        summary = {
            ...
            "critical_priority": len([c for c in donation_candidates if c["donation_priority"] == "critical"]),
            "high_priority": len([c for c in donation_candidates if c["donation_priority"] == "high"]),
            "estimated_total_value": sum(c["estimated_donation_value"] for c in donation_candidates)
        }
        
        processing_time_ms = (time.time() - start_time) * 1000
        
        logger.info("Donation quick scan list generated",
                   store_id=store_id,
        ...
        return {
            ...
            "mobile_hints": {
                ...
                "temperature_monitoring_needed": any(c["temperature_sensitive"] for c in donation_candidates),
        ...
        processing_time_ms = (time.time() - start_time) * 1000
        logger.error("Donation quick scan list failed",
                    store_id=store_id,
                    error=str(e),
                    processing_time_ms=processing_time_ms,
                    user_id=current_user["sub"])
        raise HTTPException(
            status_code=500,
            detail="Failed to generate donation scan list"
        )
