...
async def score_store_batch(
    store_id: str,
    request: Request,
    force_recalculate: bool = Query(False, description="Force recalculation of all scores"),
    ...
    try:
        ...
        logger.info("Store inventory scored securely", 
                   store_id=store_id, 
        ...
        logger.error("Failed to score store inventory", 
                    store_id=store_id, 
                    error=str(e),
                    user_id=current_user["sub"])
...
async def get_urgency_alerts(
    store_id: str,
    request: Request,
    threshold: float = Query(0.6, ge=0.0, le=1.0, description="Urgency threshold (0.0-1.0)"),
    ...
    try:
        ...
        for item in inventory_data:
            ...
            if urgency_score >= threshold:
                alerts.append({
                    ...
                    "potential_loss": item["current_quantity"] * item["selling_price"],
                    "recommendation": "Immediate action required" if urgency_score >= 0.8 else "Action needed soon"
                })
        ...
        logger.info("Urgency alerts retrieved", 
                   store_id=store_id, 
        ...
        logger.error("Failed to get urgency alerts", 
                    store_id=store_id, 
                    error=str(e),
                    user_id=current_user["sub"])
...
async def get_ai_recommendations(
    ...
    try:
        ...
        if category:
            inventory_data = [item for item in inventory_data if item["category"] == category]
        ...
        for item in inventory_data:
            ...
            if days_to_expiry <= 1:
                recommendations.append({
                    ...
                    "priority": "critical",
                    "suggested_discount": min(50, max(20, int(margin_percent * 0.6))),
                    "potential_savings": item["current_quantity"] * item["selling_price"] * 0.8
                })
            elif days_to_expiry <= 3:
                recommendations.append({
                    "batch_id": item["batch_id"],
                    ...
                    "reason": f"Expires in {days_to_expiry} day(s)",
                    "priority": "high",
                    "suggested_discount": min(25, max(10, int(margin_percent * 0.4))),
                    "potential_savings": item["current_quantity"] * item["selling_price"] * 0.6
                })
                ...
                recommendations.append({
                    ...
                    "potential_savings": item["current_quantity"] * item["selling_price"] * 0.3
                })
        ...
        return {
            ...
            "ai_insights": {
                "urgent_items": len([r for r in recommendations if r["priority"] == "critical"]),
...
async def get_scoring_analytics(
    ...
    try:
        ...
        if total_value > 0:
            at_risk_value = (analytics_data.get("critical_items", 0) + analytics_data.get("high_urgency_items", 0)) * (total_value / analytics_data.get("total_batches", 1))
        ...
        logger.info("Scoring analytics retrieved", 
                   store_id=store_id, 
                   period_days=days,
        ...
        return {
            ...
            "summary": {
                ...
                "items_at_risk": analytics_data.get("critical_items", 0) + analytics_data.get("high_urgency_items", 0),
                "total_value": total_value,
                "risk_percentage": (analytics_data.get("critical_items", 0) + analytics_data.get("high_urgency_items", 0)) / max(analytics_data.get("total_batches", 1), 1) * 100
            }
        }
        
    except Exception as e:
        logger.error("Failed to get scoring analytics", 
                    store_id=store_id, 
                    error=str(e),
                    user_id=current_user["sub"])
