...
class GlobalProduct(Base):
    ...
    dietary_attributes = Column(JSONB, default={})  # {"vegan": true, "gluten_free": false}
    ...
    verification_status = Column(String(20), default='pending')  # pending, verified, flagged
    
    # Audit fields
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    created_by = Column(UUID(as_uuid=True), ForeignKey("auth.users.id"))  # First user who added this product
    
    # Relationships
    store_products = relationship("StoreProduct", back_populates="global_product")
    batches = relationship("Batch", back_populates="global_product", foreign_keys="[Batch.global_product_id]")
...
class StoreProduct(Base):
    ...
    product_id = Column(UUID(as_uuid=True), ForeignKey("global.products.product_id"), primary_key=True)
