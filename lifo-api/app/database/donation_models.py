...
"""
from sqlalchemy import Column, String, Integer, Float, Boolean, DateTime, Date, Text, ForeignKey, Enum as SQLEnum, Decimal
...
class DonationRecipient(Base):
    ...
    food_business_registration = Column(String(100), nullable=False)  # EU food business operator number
    ...
    compliance_status = Column(SQLEnum(ComplianceStatus), default=ComplianceStatus.PENDING_VALIDATION)
...
class DonationRecord(Base):
    ...
    original_value = Column(Decimal(10, 2), nullable=False)  # Original selling price * quantity
    ...
    compliance_checks = relationship("DonationComplianceCheck", back_populates="donation")
...
class DonationComplianceCheck(Base):
    ...
    check_type = Column(String(100), nullable=False)  # "initial", "pre_pickup", "delivery"
...
class DonationAnalytics(Base):
    ...
    # EU compliance metrics
    compliance_rate_percent = Column(Float, default=0)  # % of donations that were compliant
