...
from app.models.scan_models import ScanInRequest, ScanOutRequest, ScanOutAction
...
class TestImageRecognition:
    ...
    def test_analyze_image_endpoint_structure(self, auth_headers):
        ...
        response = client.post(
            f"/api/v1/image/analyze-image/{TEST_STORE_ID}",
            headers=auth_headers
        )
...
class TestErrorHandling:
    ...
    def test_invalid_store_id_format(self, auth_headers):
        ...
        response = client.get(
            "/api/v1/mobile/mobile-summary/invalid-uuid",
            headers=auth_headers
        )
...
class TestDataValidation:
    ...
    def test_scan_in_data_validation(self, auth_headers):
        ...
        test_cases = [
            ...
            {
                "data": {"product_sku": "TEST", "expiry_date": "invalid", "quantity": 1},
                "should_fail": True,
                "error_reason": "Invalid date format"
            },
            {
                "data": {"product_sku": "TEST", "expiry_date": "2024-01-01", "quantity": -1},
                "should_fail": True,
                "error_reason": "Negative quantity"
            },
            {
                "data": {"product_sku": "TEST", "expiry_date": "2024-01-01", "quantity": 0},
                "should_fail": True,
    ...
    def test_uuid_validation(self, auth_headers):
        ...
        for invalid_uuid in invalid_uuids:
            response = client.get(
                f"/api/v1/mobile/mobile-summary/{invalid_uuid}",
                headers=auth_headers
            )
            assert response.status_code == 400
...
class TestCompleteWorkflow:
    ...
    def test_complete_scan_in_to_analytics_workflow(self, auth_headers):
        ...
        workflow_steps = [
            ("POST", f"/api/v1/scan/scan-in/{TEST_STORE_ID}", {
...
if __name__ == "__main__":
    ...
    try:
        response = client.get("/api/v1/image/ml-models/status", 
