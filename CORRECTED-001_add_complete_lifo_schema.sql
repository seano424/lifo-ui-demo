-- =============================================
-- LIFO.AI Complete Database Schema Migration - CORRECTED VERSION
-- Uses Supabase auth.users instead of custom user_mgmt.users
-- Maintains all custom user metadata in auth.users.raw_user_meta_data
-- =============================================
...
-- =============================================
-- USER MANAGEMENT SCHEMA (Supporting tables only)
-- Primary users are in auth.users (Supabase's built-in table)
-- =============================================
CREATE SCHEMA IF NOT EXISTS user_mgmt;

-- Role definitions (kept for RBAC)
...
-- User role assignments (references auth.users.id)
...
-- PIN delivery tracking (references auth.users.id)
CREATE TABLE IF NOT EXISTS user_mgmt.pin_deliveries (
    delivery_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
    store_id UUID, -- Will reference business.stores(store_id)
    delivery_method VARCHAR(20) NOT NULL CHECK (delivery_method IN ('email', 'sms', 'whatsapp')),
    delivery_address VARCHAR(255),
    pin_reference VARCHAR(100),
    pin_format VARCHAR(20) DEFAULT 'numeric',
...
CREATE TABLE IF NOT EXISTS business.stores (
    ...
    -- Ownership & Access (references auth.users.id)
...
-- Store staff access and permissions (references auth.users.id)
CREATE TABLE IF NOT EXISTS business.store_users (
    store_id UUID REFERENCES business.stores(store_id) ON DELETE CASCADE,
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
    role_in_store VARCHAR(50) CHECK (role_in_store IN ('owner', 'manager', 'employee', 'viewer')) DEFAULT 'employee',
    permissions JSONB DEFAULT '{"can_upload_inventory": true, "can_apply_discounts": false, "can_view_analytics": true}',
    assigned_at TIMESTAMP DEFAULT NOW(),
    assigned_by UUID REFERENCES auth.users(id),
    is_active BOOLEAN DEFAULT TRUE,
    
    -- PIN authentication fields (added in migration 005)
    can_use_pin_auth BOOLEAN DEFAULT FALSE,
    pin_access_level VARCHAR(20) DEFAULT 'basic' CHECK (pin_access_level IN ('basic', 'admin')),
    pin_permissions JSONB DEFAULT '{}',
...
CREATE TABLE IF NOT EXISTS inventory.products (
    ...
    -- Audit (references auth.users.id)
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW(),
...
-- User management indexes (using auth.users)
...
CREATE INDEX IF NOT EXISTS idx_pin_deliveries_user ON user_mgmt.pin_deliveries(user_id);
CREATE INDEX IF NOT EXISTS idx_pin_deliveries_store ON user_mgmt.pin_deliveries(store_id);
...
-- Enable RLS on all tables (auth.users has RLS managed by Supabase)
...
BEGIN
    RETURN EXISTS (
        ...
        AND (
            ...
            (required_role = 'employee' AND su.role_in_store IN ('employee', 'viewer'))
        )
    );
END;
...
CREATE POLICY "Users can modify inventory in their stores" ON inventory.products
    FOR ALL USING (user_has_store_access(store_id, 'employee'));
...
CREATE POLICY "Users can insert scores for their stores" ON scoring.product_scores
    FOR INSERT WITH CHECK (user_has_store_access(store_id, 'employee'));

CREATE POLICY "Users can update scores for their stores" ON scoring.product_scores
    FOR UPDATE USING (user_has_store_access(store_id, 'employee'));
...
-- User roles policies
CREATE POLICY "Users can view their own roles" ON user_mgmt.user_roles
    FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "Store owners can manage user roles" ON user_mgmt.user_roles
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM business.store_users su
            JOIN business.stores s ON su.store_id = s.store_id
            WHERE s.owner_id = auth.uid()
            AND su.user_id = user_roles.user_id
        )
    );

-- PIN deliveries policies
...
CREATE POLICY "Store managers can view PIN deliveries for their stores" ON user_mgmt.pin_deliveries
    FOR SELECT USING (user_has_store_access(store_id, 'manager'));
...
INSERT INTO user_mgmt.roles (role_name, description) VALUES
('admin', 'System administrator with full access'),
('store_owner', 'Store owner with management privileges'),
('store_manager', 'Store manager with elevated permissions'),
('store_employee', 'Store employee with basic permissions'),
...
CREATE TRIGGER update_pin_deliveries_updated_at BEFORE UPDATE ON user_mgmt.pin_deliveries
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();