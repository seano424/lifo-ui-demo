      ...
      try {
        ...
        if (session?.access_token) {
          ...
          const [fastApiResponse, aiRecommendations] = await Promise.allSettled([
            fastApiClient.getStoreAlertsWithUserToken(storeId, session.access_token, {
              threshold,
              urgency: urgencyLevel || undefined,
              category: category || undefined,
              limit,
            }),
            fastApiClient.getAIRecommendationsWithUserToken(storeId, session.access_token, {
              category: category || undefined,
          ...
          const summary = {
            ...
            total_potential_loss: filteredAlerts.reduce(
              (sum, alert) => sum + alert.potential_loss,
              0,
            ),
            ...
            avg_days_to_expiry:
              filteredAlerts.length > 0
                ? Math.round(
                    filteredAlerts.reduce((sum, a) => sum + a.days_to_expiry, 0) /
          ...
          const recommendations =
            aiRecommendations.status === 'fulfilled'
        ...
        console.warn(
          '[ALERTS] FastAPI with user JWT failed, using Supabase fallback:',
          fastApiError,
        )
