...
// Modern functional approach with factory function
export const createInventoryOperations = (supabase: SupabaseClient<Database>) => {
  ...
  const getUserStores = async (userId: string): Promise<Store[]> => {
    try {
  ...
  const createStore = async (storeData: Partial<Store>, ownerId: string): Promise<Store> => {
    try {
  ...
  const findGlobalProductByBarcode = async (barcode: string): Promise<GlobalProduct | null> => {
    const { data, error } = await supabase
      .schema('inventory')
      .from('products')
      .select('*')
  ...
  const createBatchWithGlobalProduct = async (batchData: {
  ...
  const calculateShelfLife = (category: string): number => {
  ...
  return {
    ...
    addProductToStore,
    getStoreProducts,
    createBatchWithGlobalProduct,
    processCsvBatch,
    getStoreInventoryAlerts,
    getStoreInventory,
    updateBatchQuantity,
    applyDiscount,
    calculateShelfLife,
...
// Alternative: Modern class with arrow functions and better syntax
export class InventoryOperations {
  ...
  createStore = async (storeData: Partial<Store>, ownerId: string): Promise<Store> => {
    try {
  ...
  findGlobalProductByBarcode = async (barcode: string): Promise<GlobalProduct | null> => {
    const { data, error } = await this.supabase
      .schema('inventory')
      .from('products')
      .select('*')
  ...
  createGlobalProduct = async (productData: {
    name: string
    brand?: string
    barcode?: string
    primary_category: string
  ...
  createBatchWithGlobalProduct = async (batchData: {
    global_product_id: string
    store_id: string
    batch_number: string
    expiry_date: string
