    ...
    async def addProductToStore(
        ...
        try:
            ...
            params = {
                ...
                "default_selling_price": store_product_data.get("default_selling_price"),
        ...
        try:
            ...
            if days_to_expiry is not None:
                where_conditions.append(
                    "ib.expiry_date <= :expiry_threshold"
                )
                params["expiry_threshold"] = datetime.now().date() + timedelta(days=days_to_expiry)
            ...
            while offset < total_count:
                ...
                for row in result:
                    batch_data.append({
                        ...
                        "days_to_expiry": int(row.days_to_expiry) if row.days_to_expiry else None
                    })
        ...
        try:
            ...
            metrics = {
                ...
                "inventory_summary": {
                    ...
                    "potential_profit": float((row.total_selling_value or 0) - (row.total_cost_value or 0))
                },
                "expiry_analysis": {
    ...
    async def prepareMLFeatures(
        ...
        try:
            if feature_types is None:
                feature_types = ["expiry", "quantity", "pricing", "category", "velocity"]
            ...
            ml_features = {
                ...
                "schema": {
                    "numerical_features": [
                        "days_to_expiry", "current_quantity", "initial_quantity",
                        "quantity_sold", "sell_through_rate", "cost_price",
                        "selling_price", "margin_percentage", "typical_shelf_life_days",
                        "days_in_inventory"
                    ],
                    "categorical_features": [
                        "primary_category", "unit_type", "location_code"
                    ]
                }
            }
        ...
        try:
            ...
            for i, update in enumerate(updates):
                try:
                    ...
                    import re
                    if not re.match(r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$', batch_id.lower()):
                    ...
                    try:
                        ...
                        raise ValueError(f"Invalid quantity in update {i}: {update.get('new_quantity')} - {str(e)}")
            ...
            for i, update in enumerate(validated_updates):
                ...
                value_clauses.append(f"(:{batch_param}::uuid, :{quantity_param}::numeric)")
...
async def validateInventoryData(data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    required_fields = ["global_product_id", "store_id", "current_quantity", "expiry_date"]
        ...
        if "current_quantity" in record:
            try:
                ...
                if qty < 0:
                    ...
                    warnings.append(f"{record_id}: Zero quantity - consider removing batch")
