...
```bash
curl -X POST "http://localhost:8000pi/v1/csv-upload/upload" \
...
```bash
curl -X GET "http://localhost:8000/api/v1/csv-upload/template" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash
curl -X POST "http://localhost:8000/api/v1/csv-upload/validate" \
