...
class CSVProcessor:
        ...
        try:
            ...
            for col in self.required_columns:
                if col not in df.columns:
                    ...
                    if similar_cols:
                        warnings.append(f"Column '{col}' not found, but found similar: {similar_cols}. Please rename to '{col}'")
                    else:
            ...
            if 'Expiry_Date' in df.columns:
                try:
                    ...
                    if invalid_dates > 0:
                        errors.append(f"{invalid_dates} rows have invalid expiry dates. Use format YYYY-MM-DD")
                    ...
                    errors.append(f"Error parsing expiry dates: {str(e)}")
            ...
            if 'Cost_Price' in df.columns and 'Selling_Price' in df.columns:
                ...
                if len(df_clean) > 0:
                    ...
                    if negative_margin > 0:
                        warnings.append(f"{negative_margin} rows have selling price lower than cost price")
    ...
    def clean_and_normalize_data(self, csv_content: str) -> Tuple[List[Dict], List[str]]:
        ...
        if 'Product_Name' in df.columns:
            ...
            df = df[df['Product_Name'].notna() & (df['Product_Name'] != '') & (df['Product_Name'] != 'nan')]
        ...
        try:
            ...
            errors.append(f"Error processing expiry dates: {str(e)}")
            return [], errors
        ...
        if 'Batch_Number' not in df.columns or df['Batch_Number'].isna().all():
            ...
            empty_batch = df['Batch_Number'].isna() | (df['Batch_Number'] == '') | (df['Batch_Number'] == 'nan')
            df.loc[empty_batch, 'Batch_Number'] = (
                df.loc[empty_batch, 'SKU'] + '-' + 
                df.loc[empty_batch, 'Expiry_Date'].str.replace('-', '') + '-AUTO'
            )
        ...
        if 'Selling_Price' in df.columns:
            ...
            if invalid_selling.any():
                errors.append(f"Removed {invalid_selling.sum()} rows with zero or negative selling price")
        ...
        if 'Cost_Price' in df.columns and 'Selling_Price' in df.columns:
            ...
            if negative_margin.any():
                errors.append(f"Warning: {negative_margin.sum()} items have selling price lower than cost price")
    ...
    def generate_sample_csv(self, num_rows: int = 10) -> str:
        ...
        sample_categories = ['fresh_produce', 'dairy', 'bakery_fresh', 'meat_fish', 'dry_goods']
        sample_data = []
        ...
        for i in range(num_rows):
            ...
            base_data = {
                'fresh_produce': {
                    'name': ['Organic Bananas', 'Fresh Apples', 'Cherry Tomatoes', 'Lettuce', 'Carrots'][i % 5],
                    'brand': 'Fresh Farm',
                    'cost': 0.80 + (i * 0.1),
                    'selling': 1.80 + (i * 0.15),
                    'days_offset': 3 + (i % 5),
                ...
                'dairy': {
                    'name': ['Organic Milk 1L', 'Greek Yogurt', 'Cheddar Cheese', 'Fresh Eggs', 'Butter'][i % 5],
                    'brand': 'Dairy Best',
                    'cost': 1.20 + (i * 0.2),
                    'selling': 2.50 + (i * 0.3),
                    'days_offset': 7 + (i % 7),
                ...
                'bakery_fresh': {
                    'name': ['Sourdough Bread', 'Croissants', 'Muffins', 'Bagels', 'Danish'][i % 5],
            ...
            sample_data.append({
                ...
                'Brand': data['brand'],
                'Quantity': 5 + (i % 20),
                'Expiry_Date': (datetime.now() + timedelta(days=data['days_offset'])).strftime('%Y-%m-%d'),
    ...
    def validate_business_rules(self, data: List[Dict]) -> List[str]:
        ...
        if high_risk_short_expiry:
            warnings.append(f"{len(high_risk_short_expiry)} high-risk items expire within 1 day - consider immediate action")
        ...
        if low_margin_items:
            warnings.append(f"{len(low_margin_items)} items have margins below 10% - limited discount flexibility")
        ...
        if multi_batch_skus:
            warnings.append(f"{len(multi_batch_skus)} SKUs have multiple batches with different expiry dates")
...
if __name__ == "__main__":
    ...
    if json_output:
        ...
        if result['success']:
            ...
            print(f"📊 Validation: {result['validation']['row_count']} rows, {len(result['validation']['columns'])} columns")
